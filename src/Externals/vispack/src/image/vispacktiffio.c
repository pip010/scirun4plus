...
// Note: was typedef'd later in file (line 337), byte should be equivalent to unsigned char
typedef	u_char RGBvalue;

extern int makebwmap();
extern int makecmap();
...
// Note: was typedef'd at top of file, byte should be equivalent to unsigned char
...
*************/
  ...
  BWmap = NULL;
  PALmap = NULL;
  ok = gt(tif, rwidth, height, (u_long*)(raster + (rheight-height)*rwidth));
  if (BWmap)
    free((char *)BWmap);
  if (PALmap)
    free((char *)PALmap);
 ...
 */	
static
int gtTileContig(tif, raster, Map, h, w, bpp)
     TIFF *tif;
     byte *raster;
     RGBvalue *Map;
     u_long h, w;
 ...
 */	
static
int gtStripContig(tif, raster, Map, h, w, bpp)
	TIFF *tif;
	byte *raster;
	RGBvalue *Map;
	u_long h, w;
 ...
 */
static
int gtStripSeparate(tif, raster, Map, h, w, bpp)
	TIFF *tif;
	byte *raster;
