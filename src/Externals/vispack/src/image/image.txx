...
template< class T >
VISImage<T> VISImage<T>::max(T value) const
{
  VISImage<T> image_return = this->createToSize();
  for (int i = 0; i < _channels; i++)
    image_return.repRef(i)->max((this->rep(i)), value);
  return(image_return);
}

template< class T >
VISImage<T> VISImage<T>::min(T value) const
{
    
    VISImage<T> image_return = this->createToSize();
    for (int i = 0; i < _channels; i++)
      image_return.repRef(i)->min((this->rep(i)), value);
    return(image_return);
}
...
void VISImRep<T>::neg(const VISImRep<T>* a)
{
 if (compareSize(a))
    for (int i = 0; i < _size; i++)
	_buffer[i] = ::VISmin(a->_buffer[i], static_cast<T>(0));
 else
			    ...
			    unsigned int y_pos) 
{
    ...
    if (!(((x_pos + w) <= in_w)
	  &&((y_pos + h) <= in_h)))
	{
	    WARN("Image get roi out of bounds\n");
...
template <class T>
void VISImRep<T>::max(const VISImRep<T>* a, T value)
{
    const T* buf_a = a->buffer();
    T* buf_this = _buffer;

    for (int i = 0; i < _size; i++)
	*(buf_this++) = ::VISmax(*(buf_a++), value);
}

template <class T>
void VISImRep<T>::min(const VISImRep<T>* a, T value)
{
    const T* buf_a = a->buffer();
    T* buf_this = _buffer;

    for (int i = 0; i < _size; i++)
	*(buf_this++) = ::VISmin(*(buf_a++), value);
}
...
VISImage<T> VISImage<T>::becomeFlat() const
{
    ...
    float sqrt_depth = static_cast<float>( ::sqrt( (double)channels() ) );
