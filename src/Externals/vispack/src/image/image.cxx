...
//    printf("widht is %d\n", w);
    ...
    for (i = 0; i < order%2; i++)    
	{
	    previous =  -0.5f*kernel.at(1, 0);
	    for (j = 1; j < w - 1; j++)
		{
		    next = 0.5f*kernel.at(j - 1, 0)
			- 0.5f*kernel.at(j + 1, 0);
		    kernel.at(j-1, 0) = previous;
		    previous = next;
		}
	    next = 0.5f*kernel.at(j - 1, 0);
...
//    printf("height is %d\n", h);
    ...
    for (i = 0; i < order/2; i++)
	{
	    previous = kernel.at(0, 1) - 2.0f*kernel.at(0, 0);
	    for (j = 1; j < h - 1; j++)
		{
		    next = kernel.at(0, j - 1)
			+ kernel.at(0, j + 1) - 2.0f*kernel.at(0, j);
		    kernel.at(0, j-1) = previous;
		    previous = next;
		}
	    next = kernel.at(0, j - 1) - 2.0f*kernel.at(0, j);
	    kernel.at(0, j-1) = previous;
	    kernel.at(0, j) = next;	    
	}
    for (i = 0; i < order%2; i++)    
	{
	    previous =  -0.5f*kernel.at(0, 1);
	    for (j = 1; j < h - 1; j++)
		{
		    next = 0.5f*kernel.at(0, j - 1)
			- 0.5f*kernel.at(0, j + 1);
		    kernel.at(0, j-1) = previous;
		    previous = next;
		}
	    next = 0.5f*kernel.at(0, j - 1);
...
VISImage<float> gauss_row_kernel(float sigma, float window_size)
{
	...
	{
	    pos = i - center;
	    value = exp((float)(-pos*pos)/(2.0f*sigma*sigma));
	    kernel.at(i, 0) = value;
	    total += value;
	}
