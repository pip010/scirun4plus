...
float box(unsigned i, unsigned j, unsigned k) 
{
  float pos_x = fabs(i - VOL_W/2.0),
...
void gaussDiffuse(VolumeScalar &vol, float sigma) 
{
  float max_dt = 1.0/12.0, time = sigma*sigma/2.0f, extra_time;
  int iterations = floor(time/max_dt);
  ...
  for (l = 0; l < iterations+1; l++)
	...
	for (j = 0; j < h; j++) 
	  for (i = 0; i < w; i++) 
	    {
	      update.poke(i, j, k) = max_dt*-6.0f*(vol.peek(i, j, k) 
...
float tolerance, boolean debug = false)
{
  ...
  float lambda, min_lambda = 1.0e-6;
  lambda = min_lambda;
  while ((error > tolerance) &&(iter++ < max_iter)&&(lambda < 1.0e12))
    //while (error > 1.0e-4)
    {
      ...
      update = -1.0f*value*gradient/(gradient.lengthSqr() + lambda);
      new_position += update;
      ...
      if (field->checkBounds(new_position))
	{
	  ...
	  if (fabs(value) > fabs(old_value))
	    ...
	    {
	      lambda = VISmax(lambda/10.0f, (float)min_lambda);
	      gradient = params._Fx;
		      ...
		      boolean debug = false)
{
  ...
  float x, y, z;
  float value;
  if (vol.checkBounds(x = new_position[0], y = new_position[1], z = new_position[2]))
    {
      //      cout << "place 1" << endl;
...
int shortenDistance(vec<3> &ret, const vec<3> &pixel_pos, const vec<3> &surface_pos, 
		    ScalarField<3> *field, float &dt, boolean debug=false)
{
...
int minimizeDistance(vec<3> &ret, const vec<3> &pixel_pos, 
		     const vec<3> &surface_pos, ScalarField<3> *field, 
		     float tolerance, float tolerance_project, boolean debug = false)
{
  float error= FLT_MAX, old_distance, new_distance, tmp_distance;
  float dot_prod = 0.0;
  vec<3> surface_position = surface_pos, new_position, tmp_position, gradient, difference_vector;
  boolean done2, return_error_code;
  int iterations = 0;
  float dt = 0.1;
  ScalarFieldParams<3> params;
  float dt_min = 1.0e-20;
  int max_iter = 1000;

  float original_distance = (surface_position - pixel_pos).length();
  
  //  while(error > tolerance)
  while ((dot_prod < tolerance)&&(iterations++ < max_iter))
    {
      ...
      return_error_code = shortenDistance(tmp_position, pixel_pos, surface_position, field, dt);
      // put point back on surface
      //      tmp_distance = (pixel_pos - tmp_position).length();
      if (!return_error_code)
	{
	  return_error_code = constraintProject(new_position, pixel_pos, tmp_position, field, tolerance_project);
	...
	while (!done2) 
	  {
	    ...
	    if ((new_distance <= old_distance)||(dt <= dt_min)) done2 = true;
	    ...
	    {
	      field->computeScalarFieldParams(pixel_pos, params);
      ...
      if (!return_error_code)
	{
	  ...
	  dot_prod = fabs(DotProduct(difference_vector, gradient));
...
vec<3> findBoxIntersection(vec<3> pixel_pos, vec<3> gradient, float scaling)
{
  if (fabs(gradient[0]) > fabs(gradient[1]))
    if (fabs(gradient[0]) > fabs(gradient[2]))
      {
	return(pixel_pos + scaling*gradient/fabs(gradient[0]));
      }
    else
      {
	return(pixel_pos + scaling*gradient/fabs(gradient[2]));
      }
  else 
    if (fabs(gradient[1]) > fabs(gradient[2]))
      {
	return(pixel_pos + scaling*gradient/fabs(gradient[1]));
      }
    else
      {
	return(pixel_pos + scaling*gradient/fabs(gradient[2]));
...
int main(int argc, char** argv)
{
  ...
  //       field = new SplineCube(VOL_W, VOL_H, VOL_D, (0.2f/2.0f)*VOL_D);
  //     }
  ...
  //  read_raw("dt_cube.raw", vol);
  //  vol = VolumeScalar(volume_file.read_float("dt_pelvis.vol"));
  //  vol = VolumeScalar(volume_file.read_float("dt_cube.vol"));
  ...
  //  vol = VolumeScalar(volume_file.read_float("smth_dt_brain_flux.vol"));
  ...
  if (argc > args)
    sscanf(argv[args++], "%s", fname1);
  else
    sscanf("test.vol", "%s", fname1);
  ...
  //
  //  field = new IOScalarField<3>(fname1, IOScalarField<3>::APPROXIMATING);
  //  ((IOScalarField<3>*)field)->setMainIndicatorFunction(material);
  field = new SpherePlane(0.2, 0.4);
  ...
  int w = (int)(field_size[0]/scaling), h = (int)(field_size[1]/scaling), d = (int)(field_size[2]/scaling);
  vec<3> gradient, difference_vector, new_position, tmp_position, pixel_position, surface_position, bad_position;
  float value, old_distance, new_distance;
  ...
  for (k = recon_border; k < (d - recon_border); k++)
    {
      cout << "sameple slice " << k << endl;
      for (j = recon_border; j < (h - recon_border); j++)
	for (i = recon_border; i < (w - recon_border); i++)
	  {
	    pixel_position = vec<3>(scaling*(float)i, scaling*(float)j, scaling*(float)k);
  ...
  //  vol.setBorder(100.0f, recon_border);
  float next_value;
	...
	for (i = 0; i < (w - 1); i++)
	  {
		...
		if (this_value*next_value <= 0.0)
		  {
		    this_position  = vec<3>(scaling*i, scaling*j, scaling*k);
		    next_position  = vec<3>(scaling*(i + N[ll][0]), scaling*(j + N[ll][1]), scaling*(k + N[ll][2]));
		    this_point = new Point((this_position + (next_position - this_position)*((this_value)/(this_value  - next_value))));
...
//     }
  ...
  float mc_distance, grad_des_distance;
  //  float tolerance_project, tolerance = (1.0 - 5.0e-4);//1.0e-4;
  float tolerance_project, tolerance = (1.0 - 5.0e-3);
  //  tolerance_project = 1.0e-5;   // this works ok
  tolerance_project = 1.0e-4;  // is this enough
      ...
      for (j = y_lo; j < y_hi; j++)
	{
	  ...
	  for (i = x_lo; i < x_hi; i++)
	      ...
	      //	      if (field->value(pixel_position) > -1.0) 
		{
		  ...
		  debug = false;
		  pixel_position = vec<3>(scaling*i, scaling*j, scaling*k);
		  ...
		  error = 1.0;
		  dt = 0.1;
		  //		  pixel_position = vec<3>((float)i, (float)j, (float)k);

		  //cout << "this point is " << point_tmp << endl;
		  surface_position = (octree.closestPoint(&Point(pixel_position))).position(); 
		  return_error_code = constraintProject(new_position, pixel_position, surface_position, field, tolerance_project);
		  //	      cout << "befor min dist " << surface_position << pixel_position << endl;
		  if (!return_error_code)
		    return_error_code = minimizeDistance(new_position, pixel_position, new_position, field, tolerance, tolerance_project);
...
// 	      else 
// 		vol.poke(i, j, k) = -1.0f*mc_distance;
	    }
	}
    }
...
// 	for (i = 1; i < (w - 1); i++)
	  {
		...
		intersection_position = findBoxIntersection(pixel_position, gradient, scaling);
		x = intersection_position[0]; y = intersection_position[1]; z = intersection_position[2]; 
		if (vol_position.checkBounds(x, y, z))
		  {
		    ...
		    if (!return_error_code)
		      minimizeDistance(new_position, intersection_position, new_position, field, tolerance, tolerance_project);
		    else
		      {
		    ...
		    if (DotProduct(gradient2, gradient) < 0.85)
		      {
			vol.poke(i, j, k)  = 1.0f;
			//   		    cout << "pixel is " << pixel_position << endl;
			//		    cout << "surface_position is " << surface_position << endl;
			//		    vec_tmp = (pixel_position - vec<3>((float)(w/2), (float)(h/2), (float)(d/2))).normalize();
			...
			//			if (correction < 2.0*(pixel_position - intersection_position).length()) 
			  {
			    ...
			    //			    cout << "pixel pos " << pixel_position << endl;
			    //			    cout << "surf pos " << surface_position << endl;
			    cout <<  vec_tmp[0] <<  " " <<  vec_tmp[1] << " "  <<  vec_tmp[2] << " ";
			    ...
			    cout <<  vec_tmp[0] <<  " " <<  vec_tmp[1] << " "  <<  vec_tmp[2] << " " << "0.0" << endl;
			  }
  ...
  vol.setBorder(0.0f, 1);


#ifdef this_one_does_down_wind_gradient
  float regularize = 1.0e-20;
  for (k = 1; k < (d-1); k++)
    {
      cout << "about to do slice " <<  k << endl;
      for (j = 1; j < (h - 1) ; j++)
	for (i = 1; i < (w - 1); i++)
	  {
		...
		pixel_position = vec<3>((float)i+1, (float)j, (float)k);
		...
		pixel_position = vec<3>((float)i, (float)j+1, (float)k);
		difference_vector2 = pixel_position - surface_position;
		...
		pixel_position = vec<3>((float)i, (float)j, (float)k+1);
		difference_vector2 = pixel_position - surface_position;
...
// 	      }
  ...
  //      volume_file.write_float(vol_out, "dist.vol");
  cout << "vol flux min " << vol.min() << " and max " << vol.max() << endl;
  im_file.write((vol.image()).becomeFlat(), "vol_flux.fit");
  cout << "done " << endl;
