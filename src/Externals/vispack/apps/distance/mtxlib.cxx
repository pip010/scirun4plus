...
//-------------------------------------------------------------------------
template <>
vec<2>::vec( mtx_datatype inX, mtx_datatype inY )
{
  x[0] = inX; x[1] = inY;    
}

template <>
vec<3>::vec( mtx_datatype inX, mtx_datatype inY, mtx_datatype inZ )
{
  x[0] = inX; x[1] = inY; x[2] = inZ; 
}

template <>
vec<4>::vec( mtx_datatype inX, mtx_datatype inY, mtx_datatype inZ, mtx_datatype inW )
{
  x[0] = inX; x[1] = inY; x[2] = inZ; x[3] = inW; 
}

template <>
void vec<2>::set( mtx_datatype inX, mtx_datatype inY )
{
  x[0] = inX; x[1] = inY;    
}

template <>
void vec<3>::set( mtx_datatype inX, mtx_datatype inY, mtx_datatype inZ )
{
  x[0] = inX; x[1] = inY; x[2] = inZ;  
}

template <>
void vec<4>::set( mtx_datatype inX, mtx_datatype inY, mtx_datatype inZ, mtx_datatype inW )
{
  x[0] = inX; x[1] = inY; x[2] = inZ; x[3] = inW;  
}
...
// description: 
//-------------------------------------------------------------------------
// matrix<2,2>& matrix<2,2>::operator *= ( const matrix<2,2> &rhs )
// {
//   matrix<2,2> lhs(*this);
...
// matrix<3,3>& matrix<3,3>::operator *= ( const matrix<3,3> &rhs )
// {
//   matrix<3,3> lhs(*this);
...
matrix<2,2>& matrix<2,2>::invert()
{
  ...
  mtx_datatype det = m(0,0)*m(1,1) - m(0,1)*m(1,0);
...
matrix<3,3>& matrix<3,3>::invert()
{
  ...
  mtx_datatype idet = m(0,0)*m(1,1)*m(2,2) - m(0,0)*m(1,2)*m(2,1) -
...
quaternion RotationArc(const vec<3> &v0, const vec<3> &v1)
{
  ...
  mtx_datatype  d = DotProduct(v0,v1);
  mtx_datatype  s = (mtx_datatype)sqrt((1+d)*2);
  q.x = c(0) / s;
  q.y = c(1) / s;
  q.z = c(2) / s;
  q.w = s /2.0;
...
void MakeMatrix44(mtx_datatype *m, const vec<3> &v, const quaternion &q) 
{
                             ...
                             const vec<3> &dir2) 
{
	...
	// compute plane 
	vec<3> nrml = cor - cop;
	mtx_datatype fudgefactor = 1.0/(nrml.length() * 0.25f); // since trackball proportional to distance from cop
	nrml.normalize();
	mtx_datatype dist = -DotProduct(nrml,cor);
    ...
    {
		u=u - (nrml * (mtx_datatype)sqrt(1-m*m));
	}
    ...
    {
		v=v - (nrml * (mtx_datatype)sqrt(1-m*m));
	}
                              ...
                              const vec<3> &p1, const vec<3> &p2)
{
  ...
  vec<3> dif  = p2-p1;
  mtx_datatype dn= DotProduct(n,dif);
  mtx_datatype t = -(d+DotProduct(n,p1) )/dn;
  return p1 + (dif*t);
}
