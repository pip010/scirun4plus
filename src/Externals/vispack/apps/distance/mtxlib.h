...
typedef float mtx_datatype;

static inline double DegToRad(double a) { return a*M_PI/180.0;};
static inline double RadToDeg(double a) { return a*180.0/M_PI;};
...
public:
  ...
  vec() {};
  vec(mtx_datatype rhs);
  vec(mtx_datatype inX, mtx_datatype inY);
  vec(mtx_datatype inX, mtx_datatype inY, mtx_datatype inZ);
  vec(mtx_datatype inX, mtx_datatype inY, mtx_datatype inZ, mtx_datatype inW);
  vec(const vec &rhs) { *this = rhs; };

  vec& operator = (const vec &rhs);
  vec& operator = (mtx_datatype rhs);

  void set(mtx_datatype inX, mtx_datatype inY); 
  void set(mtx_datatype inX, mtx_datatype inY, mtx_datatype inZ);
  void set(mtx_datatype inX, mtx_datatype inY, mtx_datatype inZ, mtx_datatype inW);

  //--------------------------------------------------------------------
  // array indexing
  mtx_datatype& operator [] (unsigned int i) { return x[i]; };
  const mtx_datatype& operator [] (unsigned int i) const { return x[i]; };
  mtx_datatype& operator () (unsigned int i) { return x[i]; };
  const mtx_datatype& operator () (unsigned int i) const { return x[i]; };
  ...
  //--------------------------------------------------------------------
  // vector methods
  mtx_datatype length(mtx_datatype epsilon=0.0) const;
  mtx_datatype lengthSqr() const;
  vec& normalize();
  vec& normalize(mtx_datatype epsilon);
  vec& random();
...
template <int dim>
vec<dim> operator * (const vec<dim> &v, mtx_datatype f); 

template <int dim>
vec<dim> operator * (mtx_datatype f, const vec<dim> &v);
...
// vector methods
template <int dim>
mtx_datatype DotProduct(const vec<dim> &a, const vec<dim> &b);
...
public:
  ...
  matrix& operator = (mtx_datatype rhs);
  ...
  //--------------------------------------------------------------------
  // array indexing
  mtx_datatype& operator () (unsigned int i, unsigned int j) 
  { return x[i*c+j]; };
  const mtx_datatype& operator () (unsigned int i, unsigned int j) const 
  { return x[i*c+j]; };
  ...
  matrix& transpose();
  matrix& invert();
  mtx_datatype trace() const;
  mtx_datatype norm() const;
...
template <int r, int c>
matrix<r,c> operator * (const matrix<r,c> &a, mtx_datatype f); 

template <int r, int c>
matrix<r,c> operator * (mtx_datatype f, const matrix<r,c> &a);
...
{
 public:
	mtx_datatype x,y,z,w;
	quaternion(){ init(0.0,0.0,0.0,1.0); };

  inline void init(mtx_datatype xv, mtx_datatype yv, mtx_datatype zv, mtx_datatype wv) 
  { x=xv; y=yv; z=zv; w=wv; };
...
void       MakeMatrix44(mtx_datatype *m, const vec<3> &v,
                        const quaternion &q);
quaternion VirtualTrackBall(const vec<3> &cop,  const vec<3> &cor,
                            const vec<3> &dir1, const vec<3> &dir2);

vec<3> PlaneLineIntersection(const vec<3> &n, mtx_datatype d,
