...
VISVector sumOfRows(const VISMatrix  &A)
{
  int i, j, k;
  int rows = A.r(), cols = A.c();
  float total;
...
VISMatrix projectOntoPositiveSolution(const VISMatrix  &solution)
{
  ...
  std::vector<double>::reverse_iterator vector_iter;
  VISMatrix ret(rows, cols);
...
int main(int argc, char** argv)
{
  ...
  for (k = 0; k < num_objects; k++)
	...
	// the first index is always has it's points on the row and it's allways the greater of the two.
	if (i == j)
	  (distances[k][k]).poke(i, j) = 1.0f;
	else
	  (distances[k][k]).poke(i, j) = 0.0f;
  ...
  float corres_weight = (1.0/num_objects), separation_weight = 0.0;
  ...
  int num_initial_iterations = 0, num_constraint_iterations = 1000;
  for (int kk = 0; kk < num_initial_iterations + num_constraint_iterations; kk++)
    {
      if (kk > num_initial_iterations)
	separation_weight = (1.0)*power(1.01, kk - num_initial_iterations);
      else 
	separation_weight = 0.0f;

      if (separation_weight > 0.0f)
	dt = VISmin(dt_orig, 1.0f/(separation_weight*flt_tmp));
      else
	dt = dt_orig;

      cout << "separation eight " << separation_weight << endl;
      ...
      for (k = 0; k < num_objects; k++)
	{
	  ...
	  //	  cout << "**********" << endl;
	  //	  cout << "weights sum " << k << " " << sumOfRows(weights_tmp[k]) << endl;
	}
...
// 	  }
  ...
  for (k = 0; k < num_objects; k++)
      ...
      for (j = 0; j < num_labels; j++)
	{
      ...
      //      cout << "weights " << k << ": " << weights[k] << endl;
      cout << "weights sums final " << sumOfRows(weights[k]) << endl;
    }
