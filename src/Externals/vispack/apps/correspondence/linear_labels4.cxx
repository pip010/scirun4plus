...
VISMatrix computeLabelConstraintUpdate(const VISMatrix &weights, float *error = NULL)
{
  float *float_buf;
  int rows = weights.r(),  cols = weights.c();
  VISMatrix ret(rows, cols);
  float_buf = new float[rows];
  int i, j;
  for (i = 0; i < rows; i++)
    float_buf[i] = 0.0f;
  for (j = 0; j < cols; j++)
    for (i = 0; i < rows; i++)
      {
	float_buf[i] += weights.peek(i, j);
      }
  
  float max_error = 0.0f;
  for (i = 0; i < rows; i++)
    {
      float_buf[i] = VISmax(float_buf[i] - 1.0, 0.0);
      max_error = VISmax(max_error, float_buf[i]);
    }

  for (j = 0; j < cols; j++)
    for (i = 0; i < rows; i++)
      {
	ret.poke(i, j) = float_buf[i]/cols;
      }
...
int main(int argc, char** argv)
{
  ...
  for (k = 0; k < num_objects; k++)
      ...
      for (i = 0; i < num_pts; i++)
	{
      ...
      // scramble data and save it...
//       int index;
//       for (i = 0; i < num_pts; i++)
// 	{
// 	  index = (int)floor(rand1()*num_pts);
// 	  point_tmp = (points[k]).vec(i);
// 	  (points[k]).pokeROI(0, i, points[k].vec(index));
// 	  (points[k]).pokeROI(0, index, point_tmp);
// 	}
//       // save it
//       fscanf(in, "%s", filename);
//       in_pts = fopen(filename, "w");
//       for (i = 0; i < num_pts; i++)
// 	{
// 	  point_tmp = (points[k]).vec(i);
// 	  for (l = 0; l < dim; l++)
// 	    {
// 	      fprintf(in_pts, "%f ", point_tmp.peek(l));
// 	    }
// 	  fprintf(in_pts, "\n");
// 	}
//       fclose(in_pts);
  ...
  for (k = 0; k < num_objects; k++)
    for (i = 0; i < num_pts; i++)
      ...
      {
	// the first index is always has it's points on the row and it's allways the greater of the two.
	(distances[k][k]).poke(i, j) = (sqrt(num_pts)/flt_tmp)*1.0/(((points[k]).vec(i) - (points[k]).vec(j)).norm() + flt_tmp/sqrt(num_pts));
  ...
  float energy, energy_separation;
  VISMatrix matrix_tmp;
  VISMatrix matrix_mult_tmp1(num_pts, num_labels), matrix_mult_tmp2(num_pts, num_labels);
  float corres_weight = 1.0*(1.0/((num_objects-1))), separation_weight = 1.0; //separation_weight = (flt_tmp);

  float start_energy = 0.0f;
  energy_separation= 0.0f;
  for (k = 0; k < num_objects; k++)
    {
      for (l = 0; l < k; l++)
...
/* Code you want timed here */
	...
	matrix_tmp.poke(i, i) = 0.0f;
      energy_separation += separation_weight*matrix_tmp.sum();
    }
  
  start_energy *= (2.0/(num_objects*num_labels));
  energy_separation *= (2.0/(num_objects*num_labels));
  
  cout << "Corr energy " << start_energy << endl;
  cout << "Sep energy " << energy_separation << endl;
  separation_weight *= start_energy/energy_separation;
  energy_separation = start_energy;
  start_energy += energy_separation;
  cout << "Energy " << start_energy << endl;
  ...
  float label_constraint_weight = 1.0;

  int num_initial_iterations = 50;
  ...
  for (int kk = 0; ((kk < total_iterations)&&!done); kk++)
      ...
      for (k = 0; k < num_objects; k++)
	{
	  ...
	  // put the penality that keeps multiple labels from accumulating on the same point.
	  //	  update += flt_tmp*label_constraint_weight*computeLabelConstraintUpdate(weights[k]);
 	...
 	for (k = 0; k < num_objects; k++)
       ...
       //      if (kk%10 == 0)
       //	cout << "weights means " << num_labels*(weights_tmp[0].mean()) << endl;

      for (k = 0; k < num_objects; k++)
	{
	  float label_error = FLT_MAX;
	  //	  cout << "label error " << k << " " ;
	  for (int jj = 0; (jj < 40)&&(label_error > 0.05); jj++)
	    {
	      update = computeLabelConstraintUpdate(weights_tmp[k], &label_error);
	      //	      cout << label_error << " ";
	      weights_tmp[k] = projectOntoPositiveSolution(weights_tmp[k] - projectUpdateOntoLinearConstraints(update));
	    }
	  //	  cout << endl;
	}
      
      //      if (kk%10 == 0)      
	//	cout << "weights means after " << num_labels*(weights_tmp[0].mean()) << endl;
       ...
       if (kk%10 == 0)
	 {
	   ...
	   for (k = 0; k < num_objects; k++)
		 ...
		 energy += corres_weight*(weights[k].t()*distances[k][l]*weights[l]).trace();
	       // put the distance penality to keep things seperated
	       matrix_tmp = weights[k].t()*distances[k][l]*weights[k];
	       for (i = 0; i < num_labels; i++)
		 matrix_tmp.poke(i, i) = 0.0f;
	       energy_separation += separation_weight*matrix_tmp.sum();
	   ...
	   energy *= (2.0/(num_objects*num_labels));
	   energy_separation *= (2.0/(num_objects*num_labels));
  
	   cout << "Corr energy " << energy << endl;
	   cout << "Sep energy " << energy_separation << endl;
	   energy += energy_separation;
