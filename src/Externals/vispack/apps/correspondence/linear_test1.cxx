...
VISVector removeItems(const VISVector& vec, int item, const VISArray< boolean > &to_remove, int num_gone)
{
  int i, j, N = vec.n() - num_gone;
  VISVector ret(N);
  for (i = j = 0; i < N; i++, j++)
    {
      if (to_remove[j]) j++;
...
VISMatrix removeColumns(const VISMatrix& mat, int col, const VISArray< boolean > &to_remove, int num_gone)
{
  int i, j, k, M = mat.r(), N = mat.c() -num_gone;
  VISVector ret(N);
  for (i = 0, k = 0; i < N; i++, k++)
    {
      if (to_remove[j]) k++;
