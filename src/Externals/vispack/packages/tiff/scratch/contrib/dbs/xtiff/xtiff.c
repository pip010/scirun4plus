...
Boolean SearchVisualVISList PP((int image_depth,
    ...
    char ** argv;
{
    ...
    XtGetApplicationResources(shellWidget, &appData,
        (XtResourceVISList) clientResources, (Cardinal) XtNumber(clientResources),
        (ArgVISList) NULL, (Cardinal) 0);
...
GetVisual()
{
    ...
    case 32:
    case 24:
        if (SearchVisualVISList(24, DirectColor, &xVisual) == False) {
    ...
    case 2:
        ...
        case PHOTOMETRIC_MINISWHITE:
        case PHOTOMETRIC_MINISBLACK:
            if (SearchVisualVISList((int) tfImageDepth, GrayScale, &xVisual) == True)
                break;
        case PHOTOMETRIC_PALETTE:
            if (SearchVisualVISList((int) tfImageDepth, PseudoColor, &xVisual) == True)
                break;
        default:
 ...
 */
Boolean
SearchVisualVISList(image_depth, visual_class, visual)
...
SelectProc(w, unused_1, unused_2)
    ...
    caddr_t unused_2;
{
    XawVISListReturnStruct *list_return;

    list_return = XawVISListShowCurrent(w);
