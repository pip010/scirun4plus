 ...
 */
	...
	spbequ_(uplo, n, kd, &AB(1,1), ldab, &S(1), &scond, &amax, &
		infequ);
	if (infequ == 0) {

/*           Equilibrate the matrix. */

	    slaqsb_(uplo, n, kd, &AB(1,1), ldab, &S(1), &scond, &amax, 
		    equed);
	    rcequ = lsame_(equed, "Y");
	}
    }
...
/* Computing MAX */
		...
		j1 = max(i__2,1);
		i__2 = j - j1 + 1;
		scopy_(&i__2, &AB(*kd+1-j+j1,j), &c__1, &
...
/* Computing MIN */
		...
		j2 = min(i__2,*n);
		i__2 = j2 - j + 1;
		scopy_(&i__2, &AB(1,j), &c__1, &AFB(1,j), &c__1);
/* L50: */
	    }
	}

	spbtrf_(uplo, n, kd, &AFB(1,1), ldafb, info);
...
/*     Compute the norm of the matrix A. */
    ...
    anorm = slansb_("1", uplo, n, kd, &AB(1,1), ldab, &WORK(1));

/*     Compute the reciprocal of the condition number of A. */

    spbcon_(uplo, n, kd, &AFB(1,1), ldafb, &anorm, rcond, &WORK(1), &
	    IWORK(1), info);
...
/*     Compute the solution matrix X. */
    ...
    spbtrs_(uplo, n, kd, nrhs, &AFB(1,1), ldafb, &X(1,1), ldx, 
	    info);

/*     Use iterative refinement to improve the computed solution and   
       compute error bounds and backward error estimates for it. */

    spbrfs_(uplo, n, kd, nrhs, &AB(1,1), ldab, &AFB(1,1), ldafb, 
