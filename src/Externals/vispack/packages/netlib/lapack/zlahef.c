...
/*        Copy column K of A to column KW of W and update it */
	...
	i__1 = k - 1;
	zcopy_(&i__1, &A(1,k), &c__1, &W(1,kw), &c__1);
	i__1 = k + kw * w_dim1;
	i__2 = k + k * a_dim1;
	d__1 = A(k,k).r;
	W(k,kw).r = d__1, W(k,kw).i = 0.;
	if (k < *n) {
	    i__1 = *n - k;
	    z__1.r = -1., z__1.i = 0.;
...
e it */
		...
		i__1 = imax - 1;
		zcopy_(&i__1, &A(1,imax), &c__1, &W(1,kw-1), &c__1);
		i__1 = imax + (kw - 1) * w_dim1;
		i__2 = imax + imax * a_dim1;
		d__1 = A(imax,imax).r;
		W(imax,kw-1).r = d__1, W(imax,kw-1).i = 0.;
		i__1 = k - imax;
		zcopy_(&i__1, &A(imax,imax+1), lda, &W(imax+1,kw-1), &c__1);
		i__1 = k - imax;
		zlacgv_(&i__1, &W(imax+1,kw-1), &c__1);
		if (k < *n) {
		    i__1 = *n - k;
		    z__1.r = -1., z__1.i = 0.;
...
*/
		    ...
		    i__1 = imax + (kw - 1) * w_dim1;
		    if ((d__1 = W(imax,kw-1).r, abs(d__1)) >= alpha * rowmax) {
...
/*              Copy non-updated column KK to column KP */
		...
		i__2 = kk + kk * a_dim1;
		d__1 = A(kk,kk).r;
		A(kp,kp).r = d__1, A(kp,kp).i = 0.;
		i__1 = kk - 1 - kp;
		zcopy_(&i__1, &A(kp+1,kk), &c__1, &A(kp,kp+1), lda);
		i__1 = kk - 1 - kp;
		zlacgv_(&i__1, &A(kp,kp+1), lda);
		i__1 = kp - 1;
		zcopy_(&i__1, &A(1,kk), &c__1, &A(1,kp),
			 &c__1);
...
of A and W */
		...
		if (kk < *n) {
		    i__1 = *n - kk;
		    zswap_(&i__1, &A(kk,kk+1), lda, &A(kp,kk+1), lda);
		}
		i__1 = *n - kk + 1;
...
lds   
		...
		r1 = 1. / A(k,k).r;
		i__1 = k - 1;
		zdscal_(&i__1, &r1, &A(1,k), &c__1);

/*              Conjugate W(k) */

		i__1 = k - 1;
		zlacgv_(&i__1, &W(1,kw), &c__1);
	    } else {
...
k-1 of A */
			...
			d_cnjg(&z__2, &d21);
			i__3 = j + kw * w_dim1;
			z__4.r = d22.r * W(j,kw).r - d22.i * W(j,kw).i, 
				z__4.i = d22.r * W(j,kw).i + d22.i * W(j,kw)
				.r;
			i__4 = j + (kw - 1) * w_dim1;
			z__3.r = z__4.r - W(j,kw-1).r, z__3.i = z__4.i - W(j,kw-1)
				.i;
...
/*        Copy column K of A to column K of W and update it */
	...
	i__2 = k + k * a_dim1;
	d__1 = A(k,k).r;
	W(k,k).r = d__1, W(k,k).i = 0.;
	if (k < *n) {
	    i__1 = *n - k;
	    zcopy_(&i__1, &A(k+1,k), &c__1, &W(k+1,k), &c__1);
	}
	i__1 = *n - k + 1;
	i__2 = k - 1;
	z__1.r = -1., z__1.i = 0.;
...
/*              Copy column IMAX to column K+1 of W and update
 it */
		...
		i__1 = imax - k;
		zlacgv_(&i__1, &W(k,k+1), &c__1);
		i__1 = imax + (k + 1) * w_dim1;
		i__2 = imax + imax * a_dim1;
		d__1 = A(imax,imax).r;
		W(imax,k+1).r = d__1, W(imax,k+1).i = 0.;
		if (imax < *n) {
		    i__1 = *n - imax;
		    zcopy_(&i__1, &A(imax+1,imax), &c__1, &W(imax+1,k+1), &c__1);
		}
		i__1 = *n - k + 1;
		i__2 = k - 1;
		z__1.r = -1., z__1.i = 0.;
...
*/
		    ...
		    i__1 = imax + (k + 1) * w_dim1;
		    if ((d__1 = W(imax,k+1).r, abs(d__1)) >= alpha * rowmax) {
...
/*              Copy non-updated column KK to column KP */
		...
		A(kp,kp).r = d__1, A(kp,kp).i = 0.;
		i__1 = kp - kk - 1;
		zcopy_(&i__1, &A(kk+1,kk), &c__1, &A(kp,kk+1), lda);
		i__1 = kp - kk - 1;
		zlacgv_(&i__1, &A(kp,kk+1), lda);
		if (kp < *n) {
		    i__1 = *n - kp;
		    zcopy_(&i__1, &A(kp+1,kk), &c__1, &A(kp+1,kp), &c__1);
		}
...
/*              Interchange rows KK and KP in first KK columns
 of A and W */
		...
		i__1 = kk - 1;
		zswap_(&i__1, &A(kk,1), lda, &A(kp,1), lda);
...
ds   
		...
		if (k < *n) {
		    ...
		    r1 = 1. / A(k,k).r;
		    i__1 = *n - k;
		    zdscal_(&i__1, &r1, &A(k+1,k), &c__1);

/*                 Conjugate W(k) */

		    i__1 = *n - k;
		    zlacgv_(&i__1, &W(k+1,k), &c__1);
		}
	    } else {
...
k+1 of A */
			...
			d_cnjg(&z__2, &d21);
			i__3 = j + k * w_dim1;
			z__4.r = d11.r * W(j,k).r - d11.i * W(j,k).i, 
				z__4.i = d11.r * W(j,k).i + d11.i * W(j,k)
				.r;
			i__4 = j + (k + 1) * w_dim1;
			z__3.r = z__4.r - W(j,k+1).r, z__3.i = z__4.i - W(j,k+1)
				.i;
			z__1.r = z__2.r * z__3.r - z__2.i * z__3.i, z__1.i = 
				z__2.r * z__3.i + z__2.i * z__3.r;
			A(j,k).r = z__1.r, A(j,k).i = z__1.i;
