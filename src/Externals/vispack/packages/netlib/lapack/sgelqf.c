...
/*        Determine when to cross over from blocked to unblocked code.
   ...
   Computing MAX */
	i__1 = 0, i__2 = ilaenv_(&c__3, "SGELQF", " ", m, n, &c_n1, &c_n1, 6L,
		 1L);
	nx = max(i__1,i__2);
	if (nx < k) {
...
/*           Compute the LQ factorization of the current block   
	    ...
	    i__3 = *n - i + 1;
	    sgelq2_(&ib, &i__3, &A(i,i), lda, &TAU(i), &WORK(1), &
		    iinfo);
	    if (i + ib <= *m) {
...
tor   
		...
		i__3 = *n - i + 1;
		slarft_("Forward", "Rowwise", &i__3, &ib, &A(i,i), 
...
/*     Use unblocked code to factor the last or only block. */
	...
	i__2 = *m - i + 1;
	i__1 = *n - i + 1;
	sgelq2_(&i__2, &i__1, &A(i,i), lda, &TAU(i), &WORK(1), &
		iinfo);
    }
