...
/* Computing MAX */
			...
			WORK(i) += (r__1 = AB(l+i,j), dabs(r__1)
				);
/* L160: */
		    }
/* L170: */
...
/* Computing MAX */
			i__2 = *k + 2 - j;
			slassq_(&i__3, &AB(max(*k+2-j,1),j), &c__1, 
				&scale, &sum);
/* L280: */
		    }
		}
...
/* Computing MIN */
			i__4 = *n - j;
			i__3 = min(i__4,*k);
			slassq_(&i__3, &AB(2,j), &c__1, &scale, &
				sum);
/* L300: */
		    }
		}
...
/* Computing MIN */
		    i__4 = *n - j + 1, i__2 = *k + 1;
		    i__3 = min(i__4,i__2);
		    slassq_(&i__3, &AB(1,j), &c__1, &scale, &sum);
/* L310: */
		}
	    }
	}
