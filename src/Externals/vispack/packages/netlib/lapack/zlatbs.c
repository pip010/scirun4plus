...
/* Computing MIN */
		i__2 = *kd, i__3 = j - 1;
		jlen = min(i__2,i__3);
		CNORM(j) = dzasum_(&jlen, &AB(*kd+1-jlen,j), &
			c__1);
/* L10: */
	    }
	} else {
...
/* Computing MIN */
		...
		jlen = min(i__2,i__3);
		if (jlen > 0) {
		    CNORM(j) = dzasum_(&jlen, &AB(2,j), &c__1);
		} else {
		    CNORM(j) = 0.;
		}
/* L20: */
...
/*        Use the Level 2 BLAS solve if the reciprocal of the bound on
	...
	ztbsv_(uplo, trans, diag, n, kd, &AB(1,1), ldab, &X(1), &c__1);
    } else {
...
essary. */
		...
		if (nounit) {
		    i__3 = maind + j * ab_dim1;
		    z__1.r = tscal * AB(maind,j).r, z__1.i = tscal * AB(maind,j).i;
...
(max(1,j-kd):j-1,j)   
   ...
   Computing MIN */
			...
			zaxpy_(&jlen, &z__1, &AB(*kd+1-jlen,j)
				, &c__1, &X(j - jlen), &c__1);
			i__3 = j - 1;
...
in(j+kd,n),j)   
   ...
   Computing MIN */
			...
			z__2.r = -X(j).r, z__2.i = -X(j).i;
			z__1.r = tscal * z__2.r, z__1.i = tscal * z__2.i;
			zaxpy_(&jlen, &z__1, &AB(2,j), &c__1, &X(
				j + 1), &c__1);
		    }
		    i__3 = *n - j;
		    i = j + izamax_(&i__3, &X(j + 1), &c__1);
		    i__3 = i;
		    xmax = (d__1 = X(i).r, abs(d__1)) + (d__2 = d_imag(&X(
			    i)), abs(d__2));
		}
/* L120: */
	    }
...
*XMAX). */
		    ...
		    if (nounit) {
			i__3 = maind + j * ab_dim1;
			z__1.r = tscal * AB(maind,j).r, z__1.i = tscal * AB(maind,j)
				.i;
			tjjs.r = z__1.r, tjjs.i = z__1.i;
		    } else {
			tjjs.r = tscal, tjjs.i = 0.;
...
/* Computing MIN */
			i__3 = *kd, i__4 = j - 1;
			jlen = min(i__3,i__4);
			zdotu_(&z__1, &jlen, &AB(*kd+1-jlen,j)
				, &c__1, &X(j - jlen), &c__1);
...
/* Computing MIN */
			...
			for (i = 1; i <= jlen; ++i) {
			    ...
			    z__3.r = AB(*kd+i-jlen,j).r * uscal.r - AB(*kd+i-jlen,j).i * 
				    uscal.i, z__3.i = AB(*kd+i-jlen,j).r * uscal.i + 
				    AB(*kd+i-jlen,j).i * uscal.r;
			    i__5 = j - jlen - 1 + i;
			    z__2.r = z__3.r * X(j-jlen-1+i).r - z__3.i * X(j-jlen-1+i).i, 
				    z__2.i = z__3.r * X(j-jlen-1+i).i + z__3.i * X(
				    j-jlen-1+i).r;
...
/* Computing MIN */
			...
			for (i = 1; i <= jlen; ++i) {
			    i__4 = i + 1 + j * ab_dim1;
			    z__3.r = AB(i+1,j).r * uscal.r - AB(i+1,j).i * 
				    uscal.i, z__3.i = AB(i+1,j).r * uscal.i + 
				    AB(i+1,j).i * uscal.r;
			    i__5 = j + i;
...
/* Computing MIN */
			i__3 = *kd, i__4 = j - 1;
			jlen = min(i__3,i__4);
			zdotc_(&z__1, &jlen, &AB(*kd+1-jlen,j)
				, &c__1, &X(j - jlen), &c__1);
			csumj.r = z__1.r, csumj.i = z__1.i;
		    } else {
/* Computing MIN */
