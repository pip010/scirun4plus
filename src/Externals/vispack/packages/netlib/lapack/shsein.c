...
/* Subroutine */ int shsein_(char *side, char *eigsrc, char *initv, logical *
	select, integer *n, real *h, integer *ldh, real *wr, real *wi, real *
...
KR) if it   
		...
		i__2 = kr - kl + 1;
		hnorm = slanhs_("I", &i__2, &H(kl,kl), ldh, &WORK(
			1));
		if (hnorm > 0.f) {
		    eps3 = hnorm * ulp;
		} else {
...
L60:
	    i__2 = kl;
	    for (i = k - 1; i >= kl; --i) {
		if (SELECT(i) && (r__1 = WR(i) - wkr, dabs(r__1)) + (r__2 = 
			WI(i) - wki, dabs(r__2)) < eps3) {
		    wkr += eps3;
		    goto L60;
		}
/* L70: */
...
/*              Compute left eigenvector. */
		...
		i__2 = *n - kl + 1;
		slaein_(&c_false, &noinit, &i__2, &H(kl,kl), ldh, &
...
/*              Compute right eigenvector. */
		...
		slaein_(&c_true, &noinit, &kr, &H(1,1), ldh, &wkr, &wki, 
