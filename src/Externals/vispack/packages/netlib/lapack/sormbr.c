...
/* Subroutine */ int sormbr_(char *vect, char *side, char *trans, integer *m, 
	integer *n, integer *k, real *a, integer *lda, real *tau, real *c, 
...
/*           Q was determined by a call to SGEBRD with nq >= k */
	    ...
	    sormqr_(side, trans, m, n, k, &A(1,1), lda, &TAU(1), &C(1,1), ldc, &WORK(1), lwork, &iinfo);
	} else if (nq > 1) {
...
/*           P was determined by a call to SGEBRD with nq > k */
	    ...
	    sormlq_(side, transt, m, n, k, &A(1,1), lda, &TAU(1), &C(1,1), ldc, &WORK(1), lwork, &iinfo);
	} else if (nq > 1) {
...
/*           P was determined by a call to SGEBRD with nq <= k */
	    ...
	    i__1 = nq - 1;
	    sormlq_(side, transt, &mi, &ni, &i__1, &A(1,2), lda,
