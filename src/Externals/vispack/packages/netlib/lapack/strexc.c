...
/*     Determine the first row of specified block   
    ...
    if (*ifst > 1) {
	if (T(*ifst,*ifst-1) != 0.f) {
	    --(*ifst);
	}
    }
    nbf = 1;
    if (*ifst < *n) {
	if (T(*ifst+1,*ifst) != 0.f) {
	    nbf = 2;
	}
    }
...
/*     Determine the first row of the final block   
    ...
    if (*ilst > 1) {
	if (T(*ilst,*ilst-1) != 0.f) {
	    --(*ilst);
	}
    }
    nbl = 1;
    if (*ilst < *n) {
	if (T(*ilst+1,*ilst) != 0.f) {
	    nbl = 2;
	}
    }
...
/*           Current block either 1 by 1 or 2 by 2 */
	    ...
	    nbnext = 1;
	    if (here + nbf + 1 <= *n) {
		if (T(here+nbf+1,here+nbf) != 0.f) {
		    nbnext = 2;
		}
	    }
	    slaexc_(&wantq, n, &T(1,1), ldt, &Q(1,1), ldq, &here, &
...
hich   
	    ...
	    nbnext = 1;
	    if (here + 3 <= *n) {
		if (T(here+3,here+2) != 0.f) {
		    nbnext = 2;
		}
	    }
	    i__1 = here + 1;
	    slaexc_(&wantq, n, &T(1,1), ldt, &Q(1,1), ldq, &i__1, &
...
*/
		...
		slaexc_(&wantq, n, &T(1,1), ldt, &Q(1,1), ldq, &
