...
/* Subroutine */ int cgbsvx_(char *fact, char *trans, integer *n, integer *kl,
	 ...
	 integer *ldafb, integer *ipiv, char *equed, real *r, real *c, 
 ...
 */
	...
	cgbequ_(n, n, kl, ku, &AB(1,1), ldab, &R(1), &C(1), &rowcnd, &
		colcnd, &amax, &infequ);
	if (infequ == 0) {

/*           Equilibrate the matrix. */

	    claqgb_(n, n, kl, ku, &AB(1,1), ldab, &R(1), &C(1), &rowcnd,
		     &colcnd, &amax, equed);
	    rowequ = lsame_(equed, "R") || lsame_(equed, "B");
	    colequ = lsame_(equed, "C") || lsame_(equed, "B");
	}
...
/*     Scale the right hand side. */
	...
	if (rowequ) {
		...
		for (i = 1; i <= *n; ++i) {
		    ...
		    i__4 = i;
		    i__5 = i + j * b_dim1;
		    q__1.r = R(i) * B(i,j).r, q__1.i = R(i) * B(i,j)
			    .i;
...
/* L40: */
	...
	for (j = 1; j <= *nrhs; ++j) {
		...
		i__4 = i;
		i__5 = i + j * b_dim1;
		q__1.r = C(i) * B(i,j).r, q__1.i = C(i) * B(i,j).i;
		B(i,j).r = q__1.r, B(i,j).i = q__1.i;
/* L50: */
	    }
/* L60: */
...
/* Computing MIN */
	    ...
	    j2 = min(i__2,*n);
	    i__2 = j2 - j1 + 1;
	    ccopy_(&i__2, &AB(*ku+1-j+j1,j), &c__1, &AFB(*kl+*ku+1-j+j1,j), &c__1);
/* L70: */
	}

	cgbtrf_(n, n, kl, ku, &AFB(1,1), ldafb, &IPIV(1), info);
...
/* Computing MAX */
		...
		rpvgrw = clantb_("M", "U", "N", info, &i__1, &AFB(max(1,
...
/*     Compute the norm of the matrix A and the   
	...
	*(unsigned char *)norm = 'I';
    ...
    anorm = clangb_(norm, n, kl, ku, &AB(1,1), ldab, &RWORK(1));
    i__1 = *kl + *ku;
    rpvgrw = clantb_("M", "U", "N", n, &i__1, &AFB(1,1), ldafb, &RWORK(
	    1));
    if (rpvgrw == 0.f) {
	rpvgrw = 1.f;
    } else {
	rpvgrw = clangb_("M", n, kl, ku, &AB(1,1), ldab, &RWORK(1)) 
		/ rpvgrw;
    }

/*     Compute the reciprocal of the condition number of A. */

    cgbcon_(norm, n, kl, ku, &AFB(1,1), ldafb, &IPIV(1), &anorm, rcond,
	     &WORK(1), &RWORK(1), info);
...
/*     Compute the solution matrix X. */
    ...
    cgbtrs_(trans, n, kl, ku, nrhs, &AFB(1,1), ldafb, &IPIV(1), &X(1,1), ldx, info);

/*     Use iterative refinement to improve the computed solution and   
       compute error bounds and backward error estimates for it. */

    cgbrfs_(trans, n, kl, ku, nrhs, &AB(1,1), ldab, &AFB(1,1), 
...
/*     Transform the solution matrix X to a solution of the original   
	...
	if (colequ) {
		...
		for (i = 1; i <= *n; ++i) {
		    ...
		    i__4 = i;
		    i__5 = i + j * x_dim1;
		    q__1.r = C(i) * X(i,j).r, q__1.i = C(i) * X(i,j)
			    .i;
...
/* L120: */
	...
	for (j = 1; j <= *nrhs; ++j) {
		...
		i__4 = i;
		i__5 = i + j * x_dim1;
		q__1.r = R(i) * X(i,j).r, q__1.i = R(i) * X(i,j).i;
		X(i,j).r = q__1.r, X(i,j).i = q__1.i;
/* L130: */
	    }
/* L140: */
