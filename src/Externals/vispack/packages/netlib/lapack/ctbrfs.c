...
/*        Compute residual R = B - op(A) * X,   
	...
	ctbmv_(uplo, trans, diag, n, kd, &AB(1,1), ldab, &WORK(1), &
		c__1);
...
/* Computing MAX */
			...
			for (i = max(1,k-*kd); i <= k; ++i) {
			    ...
			    RWORK(i) += ((r__1 = AB(*kd+1+i-k,k).r, dabs(r__1)) + (
				    r__2 = r_imag(&AB(*kd+1+i-k,k)), dabs(r__2))) * xk;
/* L30: */
			}
/* L40: */
		    }
...
/* Computing MIN */
			...
			for (i = k; i <= min(*n,k+*kd); ++i) {
			    ...
			    RWORK(i) += ((r__1 = AB(i+1-k,k).r, dabs(r__1)) + (
				    r__2 = r_imag(&AB(i+1-k,k)
				    ), dabs(r__2))) * xk;
/* L70: */
			}
/* L80: */
...
/* Computing MAX */
			...
			for (i = max(1,k-*kd); i <= k; ++i) {
			    ...
			    i__5 = i + j * x_dim1;
			    s += ((r__1 = AB(*kd+1+i-k,k).r, dabs(r__1)) + (r__2 = 
				    r_imag(&AB(*kd+1+i-k,k))
				    , dabs(r__2))) * ((r__3 = X(i,j).r, dabs(
				    r__3)) + (r__4 = r_imag(&X(i,j)
				    ), dabs(r__4)));
/* L110: */
			}
			RWORK(k) += s;
...
/* Computing MIN */
			...
			for (i = k; i <= min(*n,k+*kd); ++i) {
			    ...
			    i__4 = i + j * x_dim1;
			    s += ((r__1 = AB(i+1-k,k).r, dabs(r__1)) + (r__2 = 
				    r_imag(&AB(i+1-k,k)), 
				    dabs(r__2))) * ((r__3 = X(i,j).r, dabs(
				    r__3)) + (r__4 = r_imag(&X(i,j)
				    ), dabs(r__4)));
...
/*              Multiply by diag(W)*inv(op(A)**H). */
		...
		ctbsv_(uplo, transt, diag, n, kd, &AB(1,1), ldab, &WORK(
			1), &c__1);
		i__2 = *n;
		for (i = 1; i <= *n; ++i) {
		    i__5 = i;
...
/* L230: */
		...
		ctbsv_(uplo, transn, diag, n, kd, &AB(1,1), ldab, &WORK(
			1), &c__1);
	    }
	    goto L210;
	}
