...
/* Computing MIN */
		i__2 = *kd, i__3 = j - 1;
		jlen = min(i__2,i__3);
		CNORM(j) = sasum_(&jlen, &AB(*kd+1-jlen,j), &
			c__1);
/* L10: */
	    }
	} else {
...
/* Computing MIN */
		...
		jlen = min(i__2,i__3);
		if (jlen > 0) {
		    CNORM(j) = sasum_(&jlen, &AB(2,j), &c__1);
		} else {
		    CNORM(j) = 0.f;
		}
/* L20: */
...
/*        Use the Level 2 BLAS solve if the reciprocal of the bound on
	...
	stbsv_(uplo, trans, diag, n, kd, &AB(1,1), ldab, &X(1), &c__1);
    } else {
...
(max(1,j-kd):j-1,j)   
   ...
   Computing MIN */
			i__3 = *kd, i__4 = j - 1;
			jlen = min(i__3,i__4);
			r__1 = -(doublereal)X(j) * tscal;
			saxpy_(&jlen, &r__1, &AB(*kd+1-jlen,j)
				, &c__1, &X(j - jlen), &c__1);
			i__3 = j - 1;
			i = isamax_(&i__3, &X(1), &c__1);
			xmax = (r__1 = X(i), dabs(r__1));
		    }
		} else if (j < *n) {
...
in(j+kd,n),j)   
   ...
   Computing MIN */
		    ...
		    if (jlen > 0) {
			r__1 = -(doublereal)X(j) * tscal;
			saxpy_(&jlen, &r__1, &AB(2,j), &c__1, &X(
				j + 1), &c__1);
		    }
		    i__3 = *n - j;
		    i = j + isamax_(&i__3, &X(j + 1), &c__1);
		    xmax = (r__1 = X(i), dabs(r__1));
		}
/* L100: */
	    }
...
/* Computing MIN */
			i__3 = *kd, i__4 = j - 1;
			jlen = min(i__3,i__4);
			sumj = sdot_(&jlen, &AB(*kd+1-jlen,j),
				 &c__1, &X(j - jlen), &c__1);
		    } else {
/* Computing MIN */
			i__3 = *kd, i__4 = *n - j;
