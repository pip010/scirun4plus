...
/*     Initialize X to the unit matrix, if needed */
    ...
    if (wantx) {
	slaset_("Full", n, n, &c_b8, &c_b9, &X(1,1), ldx);
    }
...
/*              post-multiply X by inv(S(i)) */
		...
		i__3 = *n - m;
		r__1 = 1.f / bii;
		sscal_(&i__3, &r__1, &X(m+1,i), &c__1);
		if (kbt > 0) {
		    i__3 = *n - m;
		    sger_(&i__3, &kbt, &c_b20, &X(m+1,i), &c__1, 
...
+ka+1) */
		    ...
		    slartg_(&AB(k+1,i-k+*ka), &ra1, &WORK(
...
which   
	    ...
	    if (nrt > 0) {
		slargv_(&nrt, &AB(1,j2t), &inca, &WORK(j2t - m), &
...
/*              apply rotations in 1st set from the right */
		...
		i__4 = *ka - 1;
		for (l = 1; l <= *ka-1; ++l) {
		    slartv_(&nr, &AB(ka1-l,j2), &inca, &AB(*ka-l,j2+1), &inca, &WORK(*n + j2 - 
			    m), &WORK(j2 - m), &ka1);
/* L100: */
		}
...
diagonal   
		...
		slar2v_(&nr, &AB(ka1,j2), &AB(ka1,j2+1), &AB(*ka,j2+1), &inca, &WORK(
...
/*           start applying rotations in 1st set from the left */
		...
		nrt = (*n - j2 + l) / ka1;
		if (nrt > 0) {
		    slartv_(&nrt, &AB(l,j2+ka1-l), &inca, &
...
/*              post-multiply X by product of rotations in 1st
 set */
		...
		for (j = j2; ka1 < 0 ? j >= j1 : j <= j1; j += ka1) {
		    i__1 = *n - m;
		    srot_(&i__1, &X(m+1,j), &c__1, &X(m+1,j+1), &c__1, &WORK(*n + j - m), &WORK(j 
			    - m));
/* L120: */
		}
	    }
...
side the   
		...
		WORK(i - kbt) = -(doublereal)BB(kb1-kbt,i) * 
			ra1;
	    }
	}
...
/*           finish applying rotations in 2nd set from the left */
		...
		nrt = (*n - j2 + *ka + l) / ka1;
		if (nrt > 0) {
		    slartv_(&nrt, &AB(l,j2-l+1), &inca, &AB(l+1,j2-l+1), &inca, &WORK(*n 
...
ements   
		...
		slargv_(&nr, &AB(1,j2), &inca, &WORK(j2), &ka1, &
			WORK(*n + j2), &ka1);
...
/*              post-multiply X by product of rotations in 2nd
 set */
		...
		for (j = j2; ka1 < 0 ? j >= j1 : j <= j1; j += ka1) {
		    i__4 = *n - m;
		    srot_(&i__4, &X(m+1,j), &c__1, &X(m+1,j+1), &c__1, &WORK(*n + j), &WORK(j));
/* L200: */
		}
	    }
/* L210: */
...
/*              post-multiply X by inv(S(i)) */
		...
		i__4 = *n - m;
		r__1 = 1.f / bii;
		sscal_(&i__4, &r__1, &X(m+1,i), &c__1);
		if (kbt > 0) {
		    i__4 = *n - m;
		    i__3 = *ldbb - 1;
		    sger_(&i__4, &kbt, &c_b20, &X(m+1,i), &c__1, 
...
a+1,i) */
		    ...
		    slartg_(&AB(ka1-k,i), &ra1, &WORK(*n + i - 
...
which   
	    ...
	    if (nrt > 0) {
		slargv_(&nrt, &AB(ka1,j2t-*ka), &inca, &WORK(
...
/*              apply rotations in 1st set from the left */
		...
		i__1 = *ka - 1;
		for (l = 1; l <= *ka-1; ++l) {
		    slartv_(&nr, &AB(l+1,j2-l), &inca, &AB(l+2,j2-l), &inca, &WORK(*n + j2 
			    - m), &WORK(j2 - m), &ka1);
/* L330: */
		}
...
diagonal   
		...
		slar2v_(&nr, &AB(1,j2), &AB(1,j2+1), &AB(2,j2), &inca, &WORK(*n + j2 - m), 
			&WORK(j2 - m), &ka1);
...
/*           start applying rotations in 1st set from the right */
		...
		nrt = (*n - j2 + l) / ka1;
		if (nrt > 0) {
		    slartv_(&nrt, &AB(ka1-l+1,j2), &inca, &AB(ka1-l,j2+1), &inca, &WORK(*n + 
			    j2 - m), &WORK(j2 - m), &ka1);
		}
/* L340: */
	    }
...
/*              post-multiply X by product of rotations in 1st
 set */
		...
		for (j = j2; ka1 < 0 ? j >= j1 : j <= j1; j += ka1) {
		    i__2 = *n - m;
		    srot_(&i__2, &X(m+1,j), &c__1, &X(m+1,j+1), &c__1, &WORK(*n + j - m), &WORK(j 
			    - m));
/* L350: */
		}
	    }
...
side the   
		...
		WORK(i - kbt) = -(doublereal)BB(kbt+1,i-kbt)
			 * ra1;
	    }
	}
...
ements   
		...
		slargv_(&nr, &AB(ka1,j2-*ka), &inca, &WORK(j2)
			, &ka1, &WORK(*n + j2), &ka1);
...
diagonal   
		...
		slar2v_(&nr, &AB(1,j2), &AB(1,j2+1), &AB(2,j2), &inca, &WORK(*n + j2), &
			WORK(j2), &ka1);
...
/*              post-multiply X by product of rotations in 2nd
 set */
		...
		for (j = j2; ka1 < 0 ? j >= j1 : j <= j1; j += ka1) {
		    i__1 = *n - m;
		    srot_(&i__1, &X(m+1,j), &c__1, &X(m+1,j+1), &c__1, &WORK(*n + j), &WORK(j));
/* L430: */
		}
	    }
/* L440: */
...
/*              post-multiply X by inv(S(i)) */
		...
		r__1 = 1.f / bii;
		sscal_(&nx, &r__1, &X(1,i), &c__1);
		if (kbt > 0) {
		    i__3 = *ldbb - 1;
		    sger_(&nx, &kbt, &c_b20, &X(1,i), &c__1, &BB(*kb,i+1), &i__3, &X(1,i+1), ldx);
		}
	    }
...
a-1,i) */
		    ...
		    slartg_(&AB(k+1,i), &ra1, &WORK(*n + i + k 
			    - *ka), &WORK(i + k - *ka), &ra);
...
which   
	    ...
	    if (nrt > 0) {
		slargv_(&nrt, &AB(1,j1+*ka), &inca, &WORK(j1),
			 &ka1, &WORK(*n + j1), &ka1);
	    }
	    if (nr > 0) {
...
diagonal   
		...
		slar2v_(&nr, &AB(ka1,j1), &AB(ka1,j1-1), &AB(*ka,j1), &inca, &WORK(*n + 
			j1), &WORK(j1), &ka1);
...
/*           start applying rotations in 1st set from the right */
		...
		j1t = j2 - (nrt - 1) * ka1;
		if (nrt > 0) {
		    slartv_(&nrt, &AB(l,j1t), &inca, &AB(l+1,j1t-1), &inca, &WORK(*n + j1t), &
			    WORK(j1t), &ka1);
		}
/* L590: */
	    }
...
/*              post-multiply X by product of rotations in 1st
 set */
		...
		i__4 = ka1;
		for (j = j1; ka1 < 0 ? j >= j2 : j <= j2; j += ka1) {
		    srot_(&nx, &X(1,j), &c__1, &X(1,j-1), &c__1, &WORK(*n + j), &WORK(j));
/* L600: */
		}
	    }
/* L610: */
...
side the   
		...
		WORK(m - *kb + i + kbt) = -(doublereal)BB(kb1-kbt,i+kbt) * ra1;
	    }
	}
...
*/
		...
		j1t = j2 - (nrt - 1) * ka1;
		if (nrt > 0) {
		    slartv_(&nrt, &AB(l,j1t+*ka), &inca, &AB(l+1,j1t+*ka-1), &inca, &WORK(*
...
ements   
		...
		slargv_(&nr, &AB(1,j1+*ka), &inca, &WORK(m - *
...
/*           start applying rotations in 2nd set from the right */
		...
		j1t = j2 - (nrt - 1) * ka1;
		if (nrt > 0) {
		    slartv_(&nrt, &AB(l,j1t), &inca, &AB(l+1,j1t-1), &inca, &WORK(*n + m - *kb + 
...
/*              post-multiply X by product of rotations in 2nd
 set */
		...
		i__4 = ka1;
		for (j = j1; ka1 < 0 ? j >= j2 : j <= j2; j += ka1) {
		    srot_(&nx, &X(1,j), &c__1, &X(1,j-1), &c__1, &WORK(*n + m - *kb + j), &WORK(m - *
			    kb + j));
/* L680: */
		}
	    }
...
/*              post-multiply X by inv(S(i)) */
		...
		sscal_(&nx, &r__1, &X(1,i), &c__1);
		if (kbt > 0) {
		    sger_(&nx, &kbt, &c_b20, &X(1,i), &c__1, &BB(2,i), &c__1, &X(1,i+1), 
			    ldx);
		}
	    }
...
which   
	    ...
	    if (nrt > 0) {
		slargv_(&nrt, &AB(ka1,j1), &inca, &WORK(j1), &ka1,
			 &WORK(*n + j1), &ka1);
	    }
	    if (nr > 0) {
...
/*              apply rotations in 1st set from the right */
		...
		i__1 = *ka - 1;
		for (l = 1; l <= *ka-1; ++l) {
		    slartv_(&nr, &AB(l+1,j1), &inca, &AB(l+2,j1-1), &inca, &WORK(*n + j1), &
			    WORK(j1), &ka1);
/* L810: */
		}
...
diagonal   
		...
		slar2v_(&nr, &AB(1,j1), &AB(1,j1-1), &AB(2,j1-1), &inca, &WORK(*n + j1)
			, &WORK(j1), &ka1);
...
/*           start applying rotations in 1st set from the left */
		...
		j1t = j2 - (nrt - 1) * ka1;
		if (nrt > 0) {
		    slartv_(&nrt, &AB(ka1-l+1,j1t-ka1+l)
...
side the   
		...
		WORK(m - *kb + i + kbt) = -(doublereal)BB(kbt+1,i) * ra1;
	    }
	}
...
/*           finish applying rotations in 2nd set from the left */
		...
		j1t = j2 - (nrt - 1) * ka1;
		if (nrt > 0) {
		    slartv_(&nrt, &AB(ka1-l+1,j1t+l-1), 
...
ements   
		...
		slargv_(&nr, &AB(ka1,j1), &inca, &WORK(m - *kb + 
...
/*              apply rotations in 2nd set from the right */
		...
		i__4 = *ka - 1;
		for (l = 1; l <= *ka-1; ++l) {
		    slartv_(&nr, &AB(l+1,j1), &inca, &AB(l+2,j1-1), &inca, &WORK(*n + m - *kb 
			    + j1), &WORK(m - *kb + j1), &ka1);
/* L890: */
		}
...
diagonal   
		...
		slar2v_(&nr, &AB(1,j1), &AB(1,j1-1), &AB(2,j1-1), &inca, &WORK(*n + m 
