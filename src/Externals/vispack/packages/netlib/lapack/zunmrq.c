...
/*     Determine the block size.  NB may be at most NBMAX, where NBMAX   
   ...
   Writing concatenation */
    ...
    i__1 = 64, i__2 = ilaenv_(&c__1, "ZUNMRQ", ch__1, m, n, k, &c_n1, 6L, 2L);
    nb = min(i__1,i__2);
    nbmin = 2;
    ldwork = nw;
    if (nb > 1 && nb < *k) {
...
/*        Use unblocked code */
	...
	zunmr2_(side, trans, m, n, k, &A(1,1), lda, &TAU(1), &C(1,1)
		, ldc, &WORK(1), &iinfo);
    } else {
...
/*           Form the triangular factor of the block reflector   
	    ...
	    i__4 = nq - *k + i + ib - 1;
	    zlarft_("Backward", "Rowwise", &i__4, &ib, &A(i,1), lda, &
		    TAU(i), t, &c__65);
	    if (left) {
...
/*           Apply H or H' */
	    ...
	    zlarfb_(side, transt, "Backward", "Rowwise", &mi, &ni, &ib, &A(i,1), lda, t, &c__65, &C(1,1), ldc, &WORK(1), &
		    ldwork);
/* L10: */
	}
    }
