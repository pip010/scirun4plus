...
/* Subroutine */ int slahqr_(logical *wantt, logical *wantz, integer *n, 
	integer *ilo, integer *ihi, real *h, integer *ldh, real *wr, real *wi,
	 integer *iloz, integer *ihiz, real *z, integer *ldz, integer *info)
{
...
/*        Look for a single small subdiagonal element. */
	...
	for (k = i; k >= l+1; --k) {
	    ...
	    if (tst1 == 0.f) {
		i__3 = i - l + 1;
		tst1 = slanhs_("1", &i__3, &H(l,l), ldh, work);
	    }
/* Computing MAX */
	    r__2 = ulp * tst1;
...
/*           Exceptional shift. */
	    ...
	    s = (r__1 = H(i,i-1), dabs(r__1)) + (r__2 = H(i-1,i-2), dabs(r__2));
	    h44 = s * .75f;
	    h33 = h44;
	    h43h34 = s * -.4375f * s;
	} else {
...
ed.   
	...
	slanv2_(&H(i-1,i-1), &H(i-1,i), &H(i,i-1), &H(i,i), &WR(i - 1), &WI(i - 1), 
		&WR(i), &WI(i), &cs, &sn);
...
/*           Apply the transformation to the rest of H. */
	    ...
	    if (i2 > i) {
		i__1 = i2 - i;
		srot_(&i__1, &H(i-1,i+1), ldh, &H(i,i+1), ldh, &cs, &sn);
	    }
	    i__1 = i - i1 - 1;
	    srot_(&i__1, &H(i1,i-1), &c__1, &H(i1,i)
		    , &c__1, &cs, &sn);
	}
	if (*wantz) {
...
/*           Apply the transformation to Z. */
	    ...
	    srot_(&nz, &Z(*iloz,i-1), &c__1, &Z(*iloz,i), &c__1, &cs, &sn);
	}
    }
