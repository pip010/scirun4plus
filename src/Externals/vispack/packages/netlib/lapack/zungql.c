...
/*     Determine the block size. */
    ...
    nb = ilaenv_(&c__1, "ZUNGQL", " ", m, n, k, &c_n1, 6L, 1L);
    nbmin = 2;
    nx = 0;
    iws = *n;
    if (nb > 1 && nb < *k) {
...
/*        Determine when to cross over from blocked to unblocked code.
   ...
   Computing MAX */
	i__1 = 0, i__2 = ilaenv_(&c__3, "ZUNGQL", " ", m, n, k, &c_n1, 6L, 1L)
		;
	nx = max(i__1,i__2);
	if (nx < *k) {
...
/*     Use unblocked code for the first or only block. */
    ...
    i__2 = *n - kk;
    i__3 = *k - kk;
    zung2l_(&i__1, &i__2, &i__3, &A(1,1), lda, &TAU(1), &WORK(1), &iinfo)
	    ;
...
/*           Apply H to rows 1:m-k+i+ib-1 of current block */
	    ...
	    i__3 = *m - *k + i + ib - 1;
	    zung2l_(&i__3, &ib, &ib, &A(1,*n-*k+i), lda, &
		    TAU(i), &WORK(1), &iinfo);
