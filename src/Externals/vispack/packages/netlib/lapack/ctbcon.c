...
/*     Compute the 1-norm of the triangular matrix A or A'. */
    ...
    anorm = clantb_(norm, uplo, diag, n, kd, &AB(1,1), ldab, &RWORK(1));
...
/*              Multiply by inv(A). */
		...
		clatbs_(uplo, "No transpose", diag, normin, n, kd, &AB(1,1), ldab, &WORK(1), &scale, &RWORK(1), info);
	    } else {

/*              Multiply by inv(A'). */

		clatbs_(uplo, "Conjugate transpose", diag, normin, n, kd, &AB(1,1), ldab, &WORK(1), &scale, &RWORK(1), info);
	    }
...
ow. */
		...
		i__1 = ix;
		xnorm = (r__1 = WORK(ix).r, dabs(r__1)) + (r__2 = r_imag(&
			WORK(ix)), dabs(r__2));
