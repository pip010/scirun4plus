...
/* Subroutine */ int sgesvx_(char *fact, char *trans, integer *n, integer *
	...
	char *equed, real *r, real *c, real *b, integer *ldb, real *x, 
 ...
 */
	...
	sgeequ_(n, n, &A(1,1), lda, &R(1), &C(1), &rowcnd, &colcnd, &
		amax, &infequ);
	if (infequ == 0) {

/*           Equilibrate the matrix. */

	    slaqge_(n, n, &A(1,1), lda, &R(1), &C(1), &rowcnd, &colcnd, &
		    amax, equed);
	    rowequ = lsame_(equed, "R") || lsame_(equed, "B");
	    colequ = lsame_(equed, "C") || lsame_(equed, "B");
	}
...
/*        Compute the LU factorization of A. */
	...
	slacpy_("Full", n, n, &A(1,1), lda, &AF(1,1), ldaf);
	sgetrf_(n, n, &AF(1,1), ldaf, &IPIV(1), info);
...
the   
		...
		rpvgrw = slantr_("M", "U", "N", info, info, &AF(1,1), 
			ldaf, &WORK(1));
		if (rpvgrw == 0.f) {
		    rpvgrw = 1.f;
		} else {
		    rpvgrw = slange_("M", n, info, &A(1,1), lda, &WORK(1)
			    ) / rpvgrw;
		}
		WORK(1) = rpvgrw;
		*rcond = 0.f;
...
/*     Compute the norm of the matrix A and the   
	...
	*(unsigned char *)norm = 'I';
    ...
    anorm = slange_(norm, n, n, &A(1,1), lda, &WORK(1));
    rpvgrw = slantr_("M", "U", "N", n, n, &AF(1,1), ldaf, &WORK(1));
    if (rpvgrw == 0.f) {
	rpvgrw = 1.f;
    } else {
	rpvgrw = slange_("M", n, n, &A(1,1), lda, &WORK(1)) / rpvgrw;
    }

/*     Compute the reciprocal of the condition number of A. */

    sgecon_(norm, n, &AF(1,1), ldaf, &anorm, rcond, &WORK(1), &IWORK(1),
	     info);
...
/*     Compute the solution matrix X. */
    ...
    sgetrs_(trans, n, nrhs, &AF(1,1), ldaf, &IPIV(1), &X(1,1), ldx,
	     info);

/*     Use iterative refinement to improve the computed solution and   
       compute error bounds and backward error estimates for it. */

    sgerfs_(trans, n, nrhs, &A(1,1), lda, &AF(1,1), ldaf, &IPIV(1),
