...
/*        Loop until stopping criterion is satisfied.   
	...
	sgemv_(trans, n, n, &c_b15, &A(1,1), lda, &X(1,j), &
...
/*           Update solution and try again. */
	    ...
	    sgetrs_(trans, n, &c__1, &AF(1,1), ldaf, &IPIV(1), &WORK(*n 
		    + 1), n, info);
...
/*              Multiply by diag(W)*inv(op(A)**T). */
		...
		sgetrs_(transt, n, &c__1, &AF(1,1), ldaf, &IPIV(1), &
			WORK(*n + 1), n, info);
		i__2 = *n;
		for (i = 1; i <= *n; ++i) {
...
/* L120: */
		...
		sgetrs_(trans, n, &c__1, &AF(1,1), ldaf, &IPIV(1), &
			WORK(*n + 1), n, info);
	    }
	    goto L100;
	}
