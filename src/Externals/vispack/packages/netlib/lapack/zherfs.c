...
/*        Loop until stopping criterion is satisfied.   
	...
	z__1.r = -1., z__1.i = 0.;
	zhemv_(uplo, n, &z__1, &A(1,1), lda, &X(1,j), &c__1, &
		c_b1, &WORK(1), &c__1);
...
/* L40: */
		...
		i__3 = k + k * a_dim1;
		RWORK(k) = RWORK(k) + (d__1 = A(k,k).r, abs(d__1)) * xk + s;
/* L50: */
	    }
	} else {
	    i__2 = *n;
		...
		xk = (d__1 = X(k,j).r, abs(d__1)) + (d__2 = d_imag(&X(k,j)), abs(d__2));
		i__3 = k + k * a_dim1;
		RWORK(k) += (d__1 = A(k,k).r, abs(d__1)) * xk;
		i__3 = *n;
		for (i = k + 1; i <= *n; ++i) {
		    i__4 = i + k * a_dim1;
		    RWORK(i) += ((d__1 = A(i,k).r, abs(d__1)) + (d__2 = 
...
/*           Update solution and try again. */
	    ...
	    zhetrs_(uplo, n, &c__1, &AF(1,1), ldaf, &IPIV(1), &WORK(1), 
		    n, info);
...
/*              Multiply by diag(W)*inv(A'). */
		...
		zhetrs_(uplo, n, &c__1, &AF(1,1), ldaf, &IPIV(1), &WORK(
			1), n, info);
		i__2 = *n;
		for (i = 1; i <= *n; ++i) {
		    i__3 = i;
