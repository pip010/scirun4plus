...
/*        Determine when to cross over from blocked to unblocked code.
   ...
   Computing MAX */
	i__1 = 0, i__2 = ilaenv_(&c__3, "CGEQRF", " ", m, n, &c_n1, &c_n1, 6L,
		 1L);
	nx = max(i__1,i__2);
	if (nx < k) {
...
/*           Compute the QR factorization of the current block   
	    ...
	    i__3 = *m - i + 1;
	    cgeqr2_(&i__3, &ib, &A(i,i), lda, &TAU(i), &WORK(1), &
		    iinfo);
	    if (i + ib <= *n) {
...
tor   
		...
		i__3 = *m - i + 1;
		clarft_("Forward", "Columnwise", &i__3, &ib, &A(i,i), lda, &TAU(i), &WORK(1), &ldwork);
...
/*     Use unblocked code to factor the last or only block. */
	...
	i__2 = *m - i + 1;
	i__1 = *n - i + 1;
	cgeqr2_(&i__2, &i__1, &A(i,i), lda, &TAU(i), &WORK(1), &
		iinfo);
    }
