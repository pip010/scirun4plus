...
/*     QR factorization of N-by-M matrix A: A = Q*R */
    ...
    zgeqrf_(n, m, &A(1,1), lda, &TAUA(1), &WORK(1), lwork, info);
    lopt = (integer) WORK(1).r;

/*     Update B := Q'*B. */

    i__1 = min(*n,*m);
    zunmqr_("Left", "Conjugate Transpose", n, p, &i__1, &A(1,1), lda, &
	    TAUA(1), &B(1,1), ldb, &WORK(1), lwork, info);
/* Computing MAX */
...
/*     RQ factorization of N-by-P matrix B: B = T*Z. */
    ...
    zgerqf_(n, p, &B(1,1), ldb, &TAUB(1), &WORK(1), lwork, info);
/* Computing MAX */
    i__1 = lopt, i__2 = (integer) WORK(1).r;
