...
/*     Determine the block size */
    ...
    nb = ilaenv_(&c__1, "SSYTRF", uplo, n, &c_n1, &c_n1, &c_n1, 6L, 1L);
    nbmin = 2;
    ldwork = *n;
    if (nb > 1 && nb < *n) {
	iws = ldwork * nb;
...
to   
	    ...
	    slasyf_(uplo, &k, &nb, &kb, &A(1,1), lda, &IPIV(1), &WORK(1),
		     &ldwork, &iinfo);
	} else {

/*           Use unblocked code to factorize columns 1:k of A */

	    ssytf2_(uplo, &k, &A(1,1), lda, &IPIV(1), &iinfo);
	    kb = k;
	}
...
to   
	    ...
	    i__1 = *n - k + 1;
	    slasyf_(uplo, &i__1, &nb, &kb, &A(k,k), lda, &IPIV(k), 
		    &WORK(1), &ldwork, &iinfo);
	} else {
...
/*           Use unblocked code to factorize columns k:n of A */
	    ...
	    i__1 = *n - k + 1;
	    ssytf2_(uplo, &i__1, &A(k,k), lda, &IPIV(k), &iinfo);
	    kb = *n - k + 1;
	}
