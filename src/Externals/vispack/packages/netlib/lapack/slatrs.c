...
/*           A is upper triangular. */
	    ...
	    for (j = 1; j <= *n; ++j) {
		i__2 = j - 1;
		CNORM(j) = sasum_(&i__2, &A(1,j), &c__1);
/* L10: */
	    }
	} else {
...
/*           A is lower triangular. */
	    ...
	    for (j = 1; j <= *n-1; ++j) {
		i__2 = *n - j;
		CNORM(j) = sasum_(&i__2, &A(j+1,j), &c__1);
/* L20: */
	    }
	    CNORM(*n) = 0.f;
	}
...
/*        Use the Level 2 BLAS solve if the reciprocal of the bound on
	...
	strsv_(uplo, trans, diag, n, &A(1,1), lda, &X(1), &c__1);
    } else {
...
/*                    Compute the update   
 ...
 A(1:j-1,j) */
			...
			saxpy_(&i__3, &r__1, &A(1,j), &c__1, &X(1),
				 &c__1);
			i__3 = j - 1;
...
*/
		    ...
		    if (upper) {
			i__3 = j - 1;
			sumj = sdot_(&i__3, &A(1,j), &c__1, &X(1), 
				&c__1);
		    } else if (j < *n) {
			i__3 = *n - j;
			sumj = sdot_(&i__3, &A(j+1,j), &c__1, &X(
				j + 1), &c__1);
		    }
		} else {
