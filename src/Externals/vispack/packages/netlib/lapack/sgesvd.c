...
/* Writing concatenation */
    ...
    mnthr = ilaenv_(&c__6, "SGESVD", ch__1, m, n, &c__0, &c__0, 6L, 2L);
    wntua = lsame_(jobu, "A");
    wntus = lsame_(jobu, "S");
    wntuas = wntua || wntus;
    wntuo = lsame_(jobu, "O");
...
/* Computing MAX */
		    i__2 = maxwrk, i__3 = *n * 3 + (*n << 1) * ilaenv_(&c__1, 
			    "SGEBRD", " ", n, n, &c_n1, &c_n1, 6L, 1L);
		    maxwrk = max(i__2,i__3);
		    if (wntvo || wntvas) {
/* Computing MAX */
			i__2 = maxwrk, i__3 = *n * 3 + (*n - 1) * ilaenv_(&
				c__1, "SORGBR", "P", n, n, n, &c_n1, 6L, 1L);
			maxwrk = max(i__2,i__3);
		    }
...
/* Computing MAX */
		    i__2 = maxwrk, i__3 = *m * 3 + (*m << 1) * ilaenv_(&c__1, 
			    "SGEBRD", " ", m, m, &c_n1, &c_n1, 6L, 1L);
		    maxwrk = max(i__2,i__3);
		    if (wntuo || wntuas) {
/* Computing MAX */
			i__2 = maxwrk, i__3 = *m * 3 + *m * ilaenv_(&c__1, 
				"SORGBR", "Q", m, m, m, &c_n1, 6L, 1L);
			maxwrk = max(i__2,i__3);
		    }
		    maxwrk = max(maxwrk,bdspac);
/* Computing MAX */
...
/*     Scale A if max element outside range [SMLNUM,BIGNUM] */
    ...
    anrm = slange_("M", m, n, &A(1,1), lda, dum);
    iscl = 0;
...
/*              Compute A=Q*R   
		...
		i__2 = *lwork - iwork + 1;
		sgeqrf_(m, n, &A(1,1), lda, &WORK(itau), &WORK(iwork), &
			i__2, &ierr);
...
/*              Zero out below R */
		...
		i__2 = *n - 1;
		i__3 = *n - 1;
		slaset_("L", &i__2, &i__3, &c_b416, &c_b416, &A(2,1), 
			lda);
		ie = 1;
		itauq = ie + *n;
		itaup = itauq + *n;
...
/*              Bidiagonalize R in A   
		...
		i__2 = *lwork - iwork + 1;
		sgebrd_(n, n, &A(1,1), lda, &S(1), &WORK(ie), &WORK(
...
rate P'.   
		    ...
		    i__2 = *lwork - iwork + 1;
		    sorgbr_("P", n, n, n, &A(1,1), lda, &WORK(itaup), &
...
ht   
		...
		sbdsqr_("U", n, &ncvt, &c__0, &c__0, &S(1), &WORK(ie), &A(1,1), lda, dum, &c__1, dum, &c__1, &WORK(iwork), 
			info);
...
them there */
		...
		if (wntvas) {
		    slacpy_("F", n, n, &A(1,1), lda, &VT(1,1), 
			    ldvt);
		}
...
t */
		    ...
		    slacpy_("U", n, n, &A(1,1), lda, &WORK(ir), &ldwrkr);
		    i__2 = *n - 1;
		    i__3 = *n - 1;
...
N*NB) */
		    ...
		    i__2 = *lwork - iwork + 1;
		    sorgqr_(m, n, n, &A(1,1), lda, &WORK(itau), &WORK(
			    iwork), &i__2, &ierr);
		    ie = itau;
		    itauq = ie + *n;
		    itaup = itauq + *n;
...
/* Computing MIN */
			...
			sgemm_("N", "N", &chunk, n, n, &c_b438, &A(i,1)
				, lda, &WORK(ir), &ldwrkr, &c_b416, &WORK(iu),
				 &ldwrku);
			slacpy_("F", &chunk, n, &WORK(iu), &ldwrku, &A(i,1), lda);
/* L10: */
		    }
...
/*                 Bidiagonalize A   
		    ...
		    i__3 = *lwork - iwork + 1;
		    sgebrd_(m, n, &A(1,1), lda, &S(1), &WORK(ie), &WORK(
...
*/
		    ...
		    i__3 = *lwork - iwork + 1;
		    sorgbr_("Q", m, n, n, &A(1,1), lda, &WORK(itauq), &
...
/*                 Copy R to VT, zeroing out below it */
		    ...
		    slacpy_("U", n, n, &A(1,1), lda, &VT(1,1), 
			    ldvt);
		    i__3 = *n - 1;
		    i__2 = *n - 1;
		    slaset_("L", &i__3, &i__2, &c_b416, &c_b416, &VT(2,1), ldvt);
...
N+2*N*NB) */
		    ...
		    i__3 = *lwork - iwork + 1;
		    sgebrd_(n, n, &VT(1,1), ldvt, &S(1), &WORK(ie), &
			    WORK(itauq), &WORK(itaup), &WORK(iwork), &i__3, &
			    ierr);
		    slacpy_("L", n, n, &VT(1,1), ldvt, &WORK(ir), &
			    ldwrkr);
...
3*N+(N-1)*NB) */
		    ...
		    i__3 = *lwork - iwork + 1;
		    sorgbr_("P", n, n, n, &VT(1,1), ldvt, &WORK(itaup), 
			    &WORK(iwork), &i__3, &ierr);
		    iwork = ie + *n;
...
omputing right   
		    ...
		    sbdsqr_("U", n, n, n, &c__0, &S(1), &WORK(ie), &VT(1,1), ldvt, &WORK(ir), &ldwrkr, dum, &c__1, 
			    &WORK(iwork), info);
		    iu = ie + *n;
...
/*                 Copy R to VT, zeroing out below it */
		    ...
		    i__2 = *n - 1;
		    i__3 = *n - 1;
		    slaset_("L", &i__2, &i__3, &c_b416, &c_b416, &VT(2,1), ldvt);
...
nalizing R   
		    ...
		    i__2 = *lwork - iwork + 1;
		    sormbr_("Q", "R", "N", m, n, n, &VT(1,1), ldvt, &
...
g right   
		    ...
		    sbdsqr_("U", n, n, m, &c__0, &S(1), &WORK(ie), &VT(1,1), ldvt, &A(1,1), lda, dum, &c__1, &
			    WORK(iwork), info);
...
/*                    Compute A=Q*R   
 ...
 N*N+N+N*NB) */
			...
			i__2 = *lwork - iwork + 1;
			sgeqrf_(m, n, &A(1,1), lda, &WORK(itau), &WORK(
				iwork), &i__2, &ierr);

/*                    Copy R to WORK(IR), zeroing out 
below it */

			slacpy_("U", n, n, &A(1,1), lda, &WORK(ir), &
				ldwrkr);
			i__2 = *n - 1;
			i__3 = *n - 1;
...
/*                    Generate Q in A   
 ...
 N*N+N+N*NB) */
			...
			i__2 = *lwork - iwork + 1;
			sorgqr_(m, n, n, &A(1,1), lda, &WORK(itau), &
				WORK(iwork), &i__2, &ierr);
			ie = itau;
			itauq = ie + *n;
			itaup = itauq + *n;
...
/*                    Multiply Q in A by left singular
 vectors of R in   
			...
			sgemm_("N", "N", m, n, n, &c_b438, &A(1,1), lda, 
...
*NB) */
			...
			i__2 = *lwork - iwork + 1;
			sorgqr_(m, n, n, &U(1,1), ldu, &WORK(itau), &
				WORK(iwork), &i__2, &ierr);
			ie = itau;
			itauq = ie + *n;
			itaup = itauq + *n;
...
/*                    Zero out below R in A */
			...
			i__2 = *n - 1;
			i__3 = *n - 1;
			slaset_("L", &i__2, &i__3, &c_b416, &c_b416, &A(2,1), lda);
...
+2*N*NB) */
			...
			i__2 = *lwork - iwork + 1;
			sgebrd_(n, n, &A(1,1), lda, &S(1), &WORK(ie), &
				WORK(itauq), &WORK(itaup), &WORK(iwork), &
				i__2, &ierr);
...
below it */
			...
			slacpy_("U", n, n, &A(1,1), lda, &WORK(iu), &
				ldwrku);
			i__2 = *n - 1;
			i__3 = *n - 1;
...
/*                    Copy right singular vectors of R
 to A   
			...
			slacpy_("F", n, n, &WORK(ir), &ldwrkr, &A(1,1), 
				lda);
...
omputing right   
			...
			sbdsqr_("U", n, n, m, &c__0, &S(1), &WORK(ie), &A(1,1), lda, &U(1,1), ldu, dum, &c__1,
				 &WORK(iwork), info);
...
ying result to VT   
 ...
 N*N+3*N+2*N*NB) */
			...
			sgebrd_(n, n, &WORK(iu), &ldwrku, &S(1), &WORK(ie), &
				WORK(itauq), &WORK(itaup), &WORK(iwork), &
				i__2, &ierr);
			slacpy_("U", n, n, &WORK(iu), &ldwrku, &VT(1,1),
				 ldvt);
...
*NB) */
			...
			i__2 = *lwork - iwork + 1;
			sorgbr_("P", n, n, n, &VT(1,1), ldvt, &WORK(
...
T   
			...
			sbdsqr_("U", n, n, n, &c__0, &S(1), &WORK(ie), &VT(1,1), ldvt, &WORK(iu), &ldwrku, dum, &
...
omputing right   
			...
			sbdsqr_("U", n, n, m, &c__0, &S(1), &WORK(ie), &VT(1,1), ldvt, &U(1,1), ldu, dum, &
				c__1, &WORK(iwork), info);
...
/*                    Generate Q in U   
 ...
 N*N+N+M*NB) */
			...
			i__2 = *lwork - iwork + 1;
			sorgqr_(m, m, n, &U(1,1), ldu, &WORK(itau), &
				WORK(iwork), &i__2, &ierr);
			ie = itau;
			itauq = ie + *n;
			itaup = itauq + *n;
...
/*                    Multiply Q in U by left singular
 vectors of R in   
			...
			sgemm_("N", "N", m, n, n, &c_b438, &U(1,1), ldu, 
...
*/
		...
		slacpy_("L", m, n, &A(1,1), lda, &U(1,1), ldu);
		if (wntus) {
		    ncu = *n;
		}
		if (wntua) {
		...
		i__2 = *lwork - iwork + 1;
		sorgbr_("Q", m, &ncu, n, &U(1,1), ldu, &WORK(itauq), &
			WORK(iwork), &i__2, &ierr);
	    }
	    if (wntvas) {
...
*/
		...
		slacpy_("U", n, n, &A(1,1), lda, &VT(1,1), ldvt);
		i__2 = *lwork - iwork + 1;
		sorgbr_("P", n, n, n, &VT(1,1), ldvt, &WORK(itaup), &
			WORK(iwork), &i__2, &ierr);
	    }
...
e left   
		...
		i__2 = *lwork - iwork + 1;
		sorgbr_("Q", m, n, n, &A(1,1), lda, &WORK(itauq), &WORK(
			iwork), &i__2, &ierr);
	    }
	    if (wntvo) {
...
*/
		...
		i__2 = *lwork - iwork + 1;
		sorgbr_("P", n, n, n, &A(1,1), lda, &WORK(itaup), &WORK(
			iwork), &i__2, &ierr);
	    }
	    iwork = ie + *n;
	    if (wntuas || wntuo) {
...
omputing   
 ...
 singular   
		...
		sbdsqr_("U", n, &ncvt, &nru, &c__0, &S(1), &WORK(ie), &VT(1,1), ldvt, &U(1,1), ldu, dum, &c__1, &
			WORK(iwork), info);
	    } else if (! wntuo && wntvo) {
...
omputing   
 ...
 singular   
		...
		sbdsqr_("U", n, &ncvt, &nru, &c__0, &S(1), &WORK(ie), &A(1,1), lda, &U(1,1), ldu, dum, &c__1, &WORK(
			iwork), info);
	    } else {
...
omputing   
 ...
 singular   
		...
		sbdsqr_("U", n, &ncvt, &nru, &c__0, &S(1), &WORK(ie), &VT(1,1), ldvt, &A(1,1), lda, dum, &c__1, &
			WORK(iwork), info);
	    }
...
/*              Compute A=L*Q   
		...
		i__2 = *lwork - iwork + 1;
		sgelqf_(m, n, &A(1,1), lda, &WORK(itau), &WORK(iwork), &
			i__2, &ierr);
...
/*              Zero out above L */
		...
		i__2 = *m - 1;
		i__3 = *m - 1;
		slaset_("U", &i__2, &i__3, &c_b416, &c_b416, &A(1,2), lda);
		ie = 1;
		itauq = ie + *m;
		itaup = itauq + *m;
		iwork = itaup + *m;
...
/*              Bidiagonalize L in A   
		...
		i__2 = *lwork - iwork + 1;
		sgebrd_(m, m, &A(1,1), lda, &S(1), &WORK(ie), &WORK(
			itauq), &WORK(itaup), &WORK(iwork), &i__2, &ierr);
		if (wntuo || wntuas) {
...
*/
		    ...
		    i__2 = *lwork - iwork + 1;
		    sorgbr_("Q", m, m, m, &A(1,1), lda, &WORK(itauq), &
			    WORK(iwork), &i__2, &ierr);
		}
		iwork = ie + *m;
		nru = 0;
...
em there */
		...
		if (wntuas) {
		    slacpy_("F", m, m, &A(1,1), lda, &U(1,1), ldu);
		}
...
t */
		    ...
		    slacpy_("L", m, m, &A(1,1), lda, &WORK(ir), &ldwrkr);
		    i__2 = *m - 1;
		    i__3 = *m - 1;
...
M*NB) */
		    ...
		    i__2 = *lwork - iwork + 1;
		    sorglq_(m, n, m, &A(1,1), lda, &WORK(itau), &WORK(
			    iwork), &i__2, &ierr);
		    ie = itau;
		    itauq = ie + *m;
		    itaup = itauq + *m;
...
/* Computing MIN */
			...
			sgemm_("N", "N", m, &blk, m, &c_b438, &WORK(ir), &
				...
				WORK(iu), &ldwrku);
			slacpy_("F", m, &blk, &WORK(iu), &ldwrku, &A(1,i), lda);
/* L30: */
		    }
...
*/
		    ...
		    i__3 = *lwork - iwork + 1;
		    sorgbr_("P", m, n, m, &A(1,1), lda, &WORK(itaup), &
			    WORK(iwork), &i__3, &ierr);
		    iwork = ie + *m;
...
ing right   
		    ...
		    sbdsqr_("L", m, n, &c__0, &c__0, &S(1), &WORK(ie), &A(1,1), lda, dum, &c__1, dum, &c__1, &WORK(
			    iwork), info);
...
/*                 Copy L to U, zeroing about above it */
		    ...
		    slacpy_("L", m, m, &A(1,1), lda, &U(1,1), ldu);
		    i__3 = *m - 1;
		    i__2 = *m - 1;
		    slaset_("U", &i__3, &i__2, &c_b416, &c_b416, &U(1,2), ldu);
...
M+2*M*NB) */
		    ...
		    i__3 = *lwork - iwork + 1;
		    sgebrd_(m, m, &U(1,1), ldu, &S(1), &WORK(ie), &WORK(
			    itauq), &WORK(itaup), &WORK(iwork), &i__3, &ierr);
		    slacpy_("U", m, m, &U(1,1), ldu, &WORK(ir), &ldwrkr);
...
M+M*NB) */
		    ...
		    i__3 = *lwork - iwork + 1;
		    sorgbr_("Q", m, m, m, &U(1,1), ldu, &WORK(itauq), &
			    WORK(iwork), &i__3, &ierr);
		    iwork = ie + *m;
...
/*                 Copy L to U, zeroing out above it */
		    ...
		    i__2 = *m - 1;
		    i__3 = *m - 1;
		    slaset_("U", &i__2, &i__3, &c_b416, &c_b416, &U(1,2), ldu);
...
L by Q in A   
		    ...
		    i__2 = *lwork - iwork + 1;
		    sormbr_("P", "L", "T", m, n, m, &U(1,1), ldu, &WORK(
...
g right   
		    ...
		    sbdsqr_("U", m, n, m, &c__0, &S(1), &WORK(ie), &A(1,1), lda, &U(1,1), ldu, dum, &c__1, &
			    WORK(iwork), info);
...
/*                    Compute A=L*Q   
 ...
 M*M+M+M*NB) */
			...
			i__2 = *lwork - iwork + 1;
			sgelqf_(m, n, &A(1,1), lda, &WORK(itau), &WORK(
				iwork), &i__2, &ierr);

/*                    Copy L to WORK(IR), zeroing out 
above it */

			slacpy_("L", m, m, &A(1,1), lda, &WORK(ir), &
				ldwrkr);
			i__2 = *m - 1;
			i__3 = *m - 1;
			slaset_("U", &i__2, &i__3, &c_b416, &c_b416, &WORK(ir 
...
/*                    Generate Q in A   
 ...
 M*M+M+M*NB) */
			...
			i__2 = *lwork - iwork + 1;
			sorglq_(m, n, m, &A(1,1), lda, &WORK(itau), &
				WORK(iwork), &i__2, &ierr);
			ie = itau;
			itauq = ie + *m;
			itaup = itauq + *m;
...
/*                    Copy result to VT */
			...
			slacpy_("U", m, n, &A(1,1), lda, &VT(1,1), 
				ldvt);
...
*NB) */
			...
			i__2 = *lwork - iwork + 1;
			sorglq_(m, n, m, &VT(1,1), ldvt, &WORK(itau), &
				WORK(iwork), &i__2, &ierr);
			ie = itau;
			itauq = ie + *m;
			itaup = itauq + *m;
...
+2*M*NB) */
			...
			i__2 = *lwork - iwork + 1;
			sgebrd_(m, m, &A(1,1), lda, &S(1), &WORK(ie), &
				WORK(itauq), &WORK(itaup), &WORK(iwork), &
				i__2, &ierr);
...
below it */
			...
			slacpy_("L", m, m, &A(1,1), lda, &WORK(iu), &
				ldwrku);
			i__2 = *m - 1;
			i__3 = *m - 1;
...
to A   
			...
			slacpy_("F", m, m, &WORK(ir), &ldwrkr, &A(1,1), 
				lda);
...
ompute right   
			...
			sbdsqr_("U", m, n, m, &c__0, &S(1), &WORK(ie), &VT(1,1), ldvt, &A(1,1), lda, dum, &
				c__1, &WORK(iwork), info);
...
ying result to U   
 ...
 M*M+3*M+2*M*NB) */
			...
			sgebrd_(m, m, &WORK(iu), &ldwrku, &S(1), &WORK(ie), &
				WORK(itauq), &WORK(itaup), &WORK(iwork), &
				i__2, &ierr);
			slacpy_("L", m, m, &WORK(iu), &ldwrku, &U(1,1), 
				ldu);
...
+2*M*NB) */
			...
			i__2 = *lwork - iwork + 1;
			sgebrd_(m, m, &U(1,1), ldu, &S(1), &WORK(ie), &
				WORK(itauq), &WORK(itaup), &WORK(iwork), &
				i__2, &ierr);
...
omputing right   
			...
			sbdsqr_("U", m, n, m, &c__0, &S(1), &WORK(ie), &VT(1,1), ldvt, &U(1,1), ldu, dum, &
				c__1, &WORK(iwork), info);
...
/*                    Generate Q in VT   
 ...
 M*M+M+N*NB) */
			...
			i__2 = *lwork - iwork + 1;
			sorglq_(n, n, m, &VT(1,1), ldvt, &WORK(itau), &
				WORK(iwork), &i__2, &ierr);
			ie = itau;
			itauq = ie + *m;
			itaup = itauq + *m;
...
/*                    Copy right singular vectors of A
 from A to VT */
			...
			slacpy_("F", m, n, &A(1,1), lda, &VT(1,1), 
				ldvt);
...
*/
		...
		slacpy_("L", m, m, &A(1,1), lda, &U(1,1), ldu);
		i__2 = *lwork - iwork + 1;
		sorgbr_("Q", m, m, n, &U(1,1), ldu, &WORK(itauq), &WORK(
			iwork), &i__2, &ierr);
	    }
	    if (wntvas) {
 ...
 */
		...
		slacpy_("U", m, n, &A(1,1), lda, &VT(1,1), ldvt);
		if (wntva) {
		    nrvt = *n;
		}
		if (wntvs) {
		...
		i__2 = *lwork - iwork + 1;
		sorgbr_("P", &nrvt, n, m, &VT(1,1), ldvt, &WORK(itaup), 
			&WORK(iwork), &i__2, &ierr);
	    }
	    if (wntuo) {
...
*/
		...
		i__2 = *lwork - iwork + 1;
		sorgbr_("Q", m, m, n, &A(1,1), lda, &WORK(itauq), &WORK(
			iwork), &i__2, &ierr);
	    }
	    if (wntvo) {
...
omputing   
 ...
 singular   
		...
		sbdsqr_("L", m, &ncvt, &nru, &c__0, &S(1), &WORK(ie), &VT(1,1), ldvt, &U(1,1), ldu, dum, &c__1, &
			WORK(iwork), info);
	    } else if (! wntuo && wntvo) {
...
omputing   
 ...
 singular   
		...
		sbdsqr_("L", m, &ncvt, &nru, &c__0, &S(1), &WORK(ie), &A(1,1), lda, &U(1,1), ldu, dum, &c__1, &WORK(
			iwork), info);
	    } else {
...
omputing   
 ...
 singular   
		...
		sbdsqr_("L", m, &ncvt, &nru, &c__0, &S(1), &WORK(ie), &VT(1,1), ldvt, &A(1,1), lda, dum, &c__1, &
			WORK(iwork), info);
	    }
