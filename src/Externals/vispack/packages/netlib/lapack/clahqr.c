...
/* Subroutine */ int clahqr_(logical *wantt, logical *wantz, integer *n, 
	integer *ilo, integer *ihi, complex *h, integer *ldh, complex *w, 
	integer *iloz, integer *ihiz, complex *z, integer *ldz, integer *info)
{
...
/*           Exceptional shift. */
	    ...
	    i__2 = i + (i - 1) * h_dim1;
	    i__3 = i - 1 + (i - 2) * h_dim1;
	    d__1 = (r__1 = H(i,i-1).r, dabs(r__1)) + (r__2 = H(i-1,i-2).r, dabs(
		    r__2));
	    t.r = d__1, t.i = 0.f;
	} else {
...
/*           Wilkinson's shift. */
		...
		if (x.r * y.r + r_imag(&x) * r_imag(&y) < 0.f) {
		    q__1.r = -(doublereal)y.r, q__1.i = -(doublereal)y.i;
		    y.r = q__1.r, y.i = q__1.i;
		}
...
tive   
             ...
             V(2) is always real before the call to CLARFG, and he
nce   
             after the call T2 ( = T1*V(2) ) is also real. */

	    if (k > m) {
		ccopy_(&c__2, &H(k,k-1), &c__1, v, &c__1);
	    }
...
-1) remains   
		...
		for (j = m; j <= i; ++j) {
			...
			if (i2 > j) {
			    i__4 = i2 - j;
			    cscal_(&i__4, &temp, &H(j,j+1), 
				    ldh);
			}
			i__4 = j - i1;
			r_cnjg(&q__1, &temp);
			cscal_(&i__4, &q__1, &H(i1,j), &c__1);
			if (*wantz) {
			    r_cnjg(&q__1, &temp);
			    cscal_(&nz, &q__1, &Z(*iloz,j), &c__1);
			}
		    }
/* L90: */
		}
