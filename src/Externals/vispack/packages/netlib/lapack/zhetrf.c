...
/*     Determine the block size */
    ...
    nb = ilaenv_(&c__1, "ZHETRF", uplo, n, &c_n1, &c_n1, &c_n1, 6L, 1L);
    nbmin = 2;
    ldwork = *n;
    if (nb > 1 && nb < *n) {
	iws = ldwork * nb;
...
to   
	    ...
	    zlahef_(uplo, &k, &nb, &kb, &A(1,1), lda, &IPIV(1), &WORK(1),
		     n, &iinfo);
	} else {

/*           Use unblocked code to factorize columns 1:k of A */

	    zhetf2_(uplo, &k, &A(1,1), lda, &IPIV(1), &iinfo);
	    kb = k;
	}
...
to   
	    ...
	    i__1 = *n - k + 1;
	    zlahef_(uplo, &i__1, &nb, &kb, &A(k,k), lda, &IPIV(k), 
		    &WORK(1), n, &iinfo);
	} else {
...
/*           Use unblocked code to factorize columns k:n of A */
	    ...
	    i__1 = *n - k + 1;
	    zhetf2_(uplo, &i__1, &A(k,k), lda, &IPIV(k), &iinfo);
	    kb = *n - k + 1;
	}
