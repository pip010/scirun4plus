...
'   
 ...
 A*V*T */
	    ...
	    zlahrd_(ihi, &i, &ib, &A(1,i), lda, &TAU(i), t, &c__65,
		     &WORK(1), &ldwork);
...
be set   
	    ...
	    i__3 = i + ib + (i + ib - 1) * a_dim1;
	    A(i+ib,i+ib-1).r = 1., A(i+ib,i+ib-1).i = 0.;
	    i__3 = *ihi - i - ib + 1;
	    z__1.r = -1., z__1.i = 0.;
	    zgemm_("No transpose", "Conjugate transpose", ihi, &i__3, &ib, &
		    z__1, &WORK(1), &ldwork, &A(i+ib,i), lda, &
		    c_b2, &A(1,i+ib), lda);
	    i__3 = i + ib + (i + ib - 1) * a_dim1;
...
/*     Use unblocked code to reduce the rest of the matrix */
    ...
    zgehd2_(n, &i, ihi, &A(1,1), lda, &TAU(1), &WORK(1), &iinfo);
