...
.   
	    ...
	    i__1 = *n - *m;
	    zlacgv_(&i__1, &A(k,m1), lda);
	    i__1 = k + k * a_dim1;
	    alpha.r = A(k,k).r, alpha.i = A(k,k).i;
	    i__1 = *n - *m + 1;
	    zlarfg_(&i__1, &alpha, &A(k,m1), lda, &TAU(k));
	    i__1 = k + k * a_dim1;
	    A(k,k).r = alpha.r, A(k,k).i = alpha.i;
	    i__1 = k;
	    d_cnjg(&z__1, &TAU(k));
	    TAU(k).r = z__1.r, TAU(k).i = z__1.i;

	    i__1 = k;
	    if ((TAU(k).r != 0. || TAU(k).i != 0.) && k > 1) {
...
f A. */
		...
		i__1 = k - 1;
		zcopy_(&i__1, &A(1,k), &c__1, &TAU(1), &c__1);

/*              Form   w = a( k ) + B*z( k )  in TAU. */

		i__1 = k - 1;
		i__2 = *n - *m;
		zgemv_("No transpose", &i__1, &i__2, &c_b1, &A(1,m1), lda, &A(k,m1), lda, &c_b1, &TAU(1), &
			c__1);
...
/*              Now form  a( k ) := a( k ) - conjg(tau)*w   
		...
		zaxpy_(&i__1, &z__1, &TAU(1), &c__1, &A(1,k), &
			c__1);
		i__1 = k - 1;
		i__2 = *n - *m;
		d_cnjg(&z__2, &TAU(k));
		z__1.r = -z__2.r, z__1.i = -z__2.i;
		zgerc_(&i__1, &i__2, &z__1, &TAU(1), &c__1, &A(k,m1), lda, &A(1,m1), lda);
	    }
/* L20: */
	}
    }
