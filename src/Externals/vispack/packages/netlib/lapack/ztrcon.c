...
/*     Compute the norm of the triangular matrix A. */
    ...
    anorm = zlantr_(norm, uplo, diag, n, n, &A(1,1), lda, &RWORK(1));
...
/*              Multiply by inv(A). */
		...
		zlatrs_(uplo, "No transpose", diag, normin, n, &A(1,1), 
			lda, &WORK(1), &scale, &RWORK(1), info);
	    } else {

/*              Multiply by inv(A'). */

		zlatrs_(uplo, "Conjugate transpose", diag, normin, n, &A(1,1), lda, &WORK(1), &scale, &RWORK(1), info);
	    }
