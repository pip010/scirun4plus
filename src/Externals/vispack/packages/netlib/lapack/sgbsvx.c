...
/* Subroutine */ int sgbsvx_(char *fact, char *trans, integer *n, integer *kl,
	...
	integer *ldafb, integer *ipiv, char *equed, real *r, real *c, real *b,
	 integer *ldb, real *x, integer *ldx, real *rcond, real *ferr, real *
	berr, real *work, integer *iwork, integer *info)
{
 ...
 */
	...
	sgbequ_(n, n, kl, ku, &AB(1,1), ldab, &R(1), &C(1), &rowcnd, &
		colcnd, &amax, &infequ);
	if (infequ == 0) {

/*           Equilibrate the matrix. */

	    slaqgb_(n, n, kl, ku, &AB(1,1), ldab, &R(1), &C(1), &rowcnd,
		     &colcnd, &amax, equed);
	    rowequ = lsame_(equed, "R") || lsame_(equed, "B");
	    colequ = lsame_(equed, "C") || lsame_(equed, "B");
	}
...
/* Computing MIN */
	    ...
	    j2 = min(i__2,*n);
	    i__2 = j2 - j1 + 1;
	    scopy_(&i__2, &AB(*ku+1-j+j1,j), &c__1, &AFB(*kl+*ku+1-j+j1,j), &c__1);
/* L70: */
	}

	sgbtrf_(n, n, kl, ku, &AFB(1,1), ldafb, &IPIV(1), info);
...
/* Computing MAX */
		...
		rpvgrw = slantb_("M", "U", "N", info, &i__1, &AFB(max(1,
...
/*     Compute the norm of the matrix A and the   
	...
	*(unsigned char *)norm = 'I';
    ...
    anorm = slangb_(norm, n, kl, ku, &AB(1,1), ldab, &WORK(1));
    i__1 = *kl + *ku;
    rpvgrw = slantb_("M", "U", "N", n, &i__1, &AFB(1,1), ldafb, &WORK(
	    1));
    if (rpvgrw == 0.f) {
	rpvgrw = 1.f;
    } else {
	rpvgrw = slangb_("M", n, kl, ku, &AB(1,1), ldab, &WORK(1)) /
		 rpvgrw;
    }

/*     Compute the reciprocal of the condition number of A. */

    sgbcon_(norm, n, kl, ku, &AFB(1,1), ldafb, &IPIV(1), &anorm, rcond,
	     &WORK(1), &IWORK(1), info);
...
/*     Compute the solution matrix X. */
    ...
    sgbtrs_(trans, n, kl, ku, nrhs, &AFB(1,1), ldafb, &IPIV(1), &X(1,1), ldx, info);

/*     Use iterative refinement to improve the computed solution and   
       compute error bounds and backward error estimates for it. */

    sgbrfs_(trans, n, kl, ku, nrhs, &AB(1,1), ldab, &AFB(1,1), 
