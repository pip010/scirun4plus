...
/* Computing MIN */
		i__2 = *kd, i__3 = j - 1;
		jlen = min(i__2,i__3);
		CNORM(j) = scasum_(&jlen, &AB(*kd+1-jlen,j), &
			c__1);
/* L10: */
	    }
	} else {
...
/* Computing MIN */
		...
		jlen = min(i__2,i__3);
		if (jlen > 0) {
		    CNORM(j) = scasum_(&jlen, &AB(2,j), &c__1);
		} else {
		    CNORM(j) = 0.f;
		}
/* L20: */
...
l. */
		...
		tjjs.r = AB(maind,j).r, tjjs.i = AB(maind,j).i;
...
/*        Use the Level 2 BLAS solve if the reciprocal of the bound on
	...
	ctbsv_(uplo, trans, diag, n, kd, &AB(1,1), ldab, &X(1), &c__1);
    } else {
...
essary. */
		...
		if (nounit) {
		    i__3 = maind + j * ab_dim1;
		    q__1.r = tscal * AB(maind,j).r, q__1.i = tscal * AB(maind,j).i;
...
(max(1,j-kd):j-1,j)   
   ...
   Computing MIN */
			...
			caxpy_(&jlen, &q__1, &AB(*kd+1-jlen,j)
				, &c__1, &X(j - jlen), &c__1);
			i__3 = j - 1;
...
in(j+kd,n),j)   
   ...
   Computing MIN */
			...
			q__2.r = -(doublereal)X(j).r, q__2.i = -(
				doublereal)X(j).i;
			q__1.r = tscal * q__2.r, q__1.i = tscal * q__2.i;
			caxpy_(&jlen, &q__1, &AB(2,j), &c__1, &X(
				j + 1), &c__1);
		    }
		    i__3 = *n - j;
		    i = j + icamax_(&i__3, &X(j + 1), &c__1);
		    i__3 = i;
		    xmax = (r__1 = X(i).r, dabs(r__1)) + (r__2 = r_imag(&X(
			    i)), dabs(r__2));
		}
/* L110: */
	    }
...
*XMAX). */
		    ...
		    if (nounit) {
			i__3 = maind + j * ab_dim1;
			q__1.r = tscal * AB(maind,j).r, q__1.i = tscal * AB(maind,j)
				.i;
			tjjs.r = q__1.r, tjjs.i = q__1.i;
		    } else {
			tjjs.r = tscal, tjjs.i = 0.f;
...
/* Computing MIN */
			i__3 = *kd, i__4 = j - 1;
			jlen = min(i__3,i__4);
			cdotu_(&q__1, &jlen, &AB(*kd+1-jlen,j)
				, &c__1, &X(j - jlen), &c__1);
...
/* Computing MIN */
			...
			i__3 = jlen;
			for (i = 1; i <= jlen; ++i) {
			    i__4 = *kd + i - jlen + j * ab_dim1;
			    q__3.r = AB(*kd+i-jlen,j).r * uscal.r - AB(*kd+i-jlen,j).i * 
				    uscal.i, q__3.i = AB(*kd+i-jlen,j).r * uscal.i + 
				    AB(*kd+i-jlen,j).i * uscal.r;
			    i__5 = j - jlen - 1 + i;
			    q__2.r = q__3.r * X(j-jlen-1+i).r - q__3.i * X(j-jlen-1+i).i, 
				    q__2.i = q__3.r * X(j-jlen-1+i).i + q__3.i * X(
				    j-jlen-1+i).r;
...
/* Computing MIN */
			...
			for (i = 1; i <= jlen; ++i) {
			    i__4 = i + 1 + j * ab_dim1;
			    q__3.r = AB(i+1,j).r * uscal.r - AB(i+1,j).i * 
				    uscal.i, q__3.i = AB(i+1,j).r * uscal.i + 
				    AB(i+1,j).i * uscal.r;
			    i__5 = j + i;
...
/* Computing MIN */
			i__3 = *kd, i__4 = j - 1;
			jlen = min(i__3,i__4);
			cdotc_(&q__1, &jlen, &AB(*kd+1-jlen,j)
				, &c__1, &X(j - jlen), &c__1);
			csumj.r = q__1.r, csumj.i = q__1.i;
		    } else {
/* Computing MIN */
