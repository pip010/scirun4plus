...
. */
	    ...
	    if (ajj <= 0.f) {
		i__2 = *kd + 1 + j * ab_dim1;
		AB(*kd+1,j).r = ajj, AB(*kd+1,j).i = 0.f;
		goto L50;
	    }
	    ajj = sqrt(ajj);
	    i__2 = *kd + 1 + j * ab_dim1;
...
te the   
	    ...
	    r__1 = 1.f / ajj;
	    csscal_(&km, &r__1, &AB(*kd+1-km,j), &c__1);
	    cher_("Upper", &km, &c_b9, &AB(*kd+1-km,j), &c__1,
		     &AB(*kd+1,j-km), &kld);
/* L10: */
	}
...
the   
	    ...
	    if (km > 0) {
		r__1 = 1.f / ajj;
		csscal_(&km, &r__1, &AB(*kd,j+1), &kld);
		clacgv_(&km, &AB(*kd,j+1), &kld);
		cher_("Upper", &km, &c_b9, &AB(*kd,j+1), &kld,
			 &AB(*kd+1,j+1), &kld);
		clacgv_(&km, &AB(*kd,j+1), &kld);
	    }
/* L20: */
...
the   
	    ...
	    r__1 = 1.f / ajj;
	    csscal_(&km, &r__1, &AB(km+1,j-km), &kld);
	    clacgv_(&km, &AB(km+1,j-km), &kld);
	    cher_("Lower", &km, &c_b9, &AB(km+1,j-km), &kld,
		     &AB(1,j-km), &kld);
	    clacgv_(&km, &AB(km+1,j-km), &kld);
/* L30: */
	}
...
te the   
	    ...
	    if (km > 0) {
		r__1 = 1.f / ajj;
		csscal_(&km, &r__1, &AB(2,j), &c__1);
		cher_("Lower", &km, &c_b9, &AB(2,j), &c__1, &AB(1,j+1), &kld);
	    }
/* L40: */
	}
    }
