...
/* Subroutine */ int chsein_(char *side, char *eigsrc, char *initv, logical *
	select, integer *n, complex *h, integer *ldh, complex *w, complex *vl,
	 integer *ldvl, complex *vr, integer *ldvr, integer *mm, integer *m, 
	complex *work, real *rwork, integer *ifaill, integer *ifailr, integer 
	*info)
{
...
ctor. */
		...
		for (i = k; i >= kl+1; --i) {
		    i__3 = i + (i - 1) * h_dim1;
		    if (H(i,i-1).r == 0.f && H(i,i-1).i == 0.f) {
			goto L30;
		    }
/* L20: */
		}
L30:
		...
		if (k > kr) {
		    ...
		    for (i = k; i <= *n-1; ++i) {
			i__3 = i + 1 + i * h_dim1;
			if (H(i+1,i).r == 0.f && H(i+1,i).i == 0.f) {
			    goto L50;
			}
/* L40: */
		    }
...
KR) if it   
		...
		i__2 = kr - kl + 1;
		hnorm = clanhs_("I", &i__2, &H(kl,kl), ldh, &RWORK(
			1));
		if (hnorm > 0.f) {
		    eps3 = hnorm * ulp;
		} else {
...
/*           Perturb eigenvalue if it is close to any previous   
	    ...
	    i__2 = k;
	    wk.r = W(k).r, wk.i = W(k).i;
L60:
	    i__2 = kl;
	    for (i = k - 1; i >= kl; --i) {
		i__3 = i;
		q__2.r = W(i).r - wk.r, q__2.i = W(i).i - wk.i;
		q__1.r = q__2.r, q__1.i = q__2.i;
		if (SELECT(i) && (r__1 = q__1.r, dabs(r__1)) + (r__2 = r_imag(
			&q__1), dabs(r__2)) < eps3) {
...
/*              Compute left eigenvector. */
		...
		i__2 = *n - kl + 1;
		claein_(&c_false, &noinit, &i__2, &H(kl,kl), ldh, &
			wk, &VL(kl,ks), &WORK(1), &ldwork, &RWORK(
			1), &eps3, &smlnum, &iinfo);
		if (iinfo > 0) {
		    ++(*info);
		    IFAILL(ks) = k;
...
/*              Compute right eigenvector. */
		...
		claein_(&c_true, &noinit, &kr, &H(1,1), ldh, &wk, &VR(1,ks), &WORK(1), &ldwork, &RWORK(1), &eps3, &
			smlnum, &iinfo);
		if (iinfo > 0) {
		    ++(*info);
		    IFAILR(ks) = k;
