...
/*        Loop until stopping criterion is satisfied.   
	...
	z__1.r = -1., z__1.i = 0.;
	zgbmv_(trans, n, n, kl, ku, &z__1, &AB(1,1), ldab, &X(1,j), &c__1, &c_b1, &WORK(1), &c__1);
...
/* Computing MIN */
		...
		for (i = max(1,k-*ku); i <= min(*n,k+*kl); ++i) {
		    i__3 = kk + i + k * ab_dim1;
		    RWORK(i) += ((d__1 = AB(kk+i,k).r, abs(d__1)) + (d__2 = 
			    d_imag(&AB(kk+i,k)), abs(d__2))) * 
			    xk;
/* L40: */
		}
/* L50: */
...
/* Computing MIN */
		...
		for (i = max(1,k-*ku); i <= min(*n,k+*kl); ++i) {
		    i__5 = kk + i + k * ab_dim1;
		    i__3 = i + j * x_dim1;
		    s += ((d__1 = AB(kk+i,k).r, abs(d__1)) + (d__2 = d_imag(&AB(kk+i,k)), abs(d__2))) * ((d__3 = X(i,j).r, abs(d__3)) + (d__4 = d_imag(&X(i,j)), abs(d__4)));
/* L60: */
		}
		RWORK(k) += s;
/* L70: */
...
/*           Update solution and try again. */
	    ...
	    zgbtrs_(trans, n, kl, ku, &c__1, &AFB(1,1), ldafb, &IPIV(1)
		    , &WORK(1), n, info);
...
/*              Multiply by diag(W)*inv(op(A)**H). */
		...
		zgbtrs_(transt, n, kl, ku, &c__1, &AFB(1,1), ldafb, &
...
/* L120: */
		...
		zgbtrs_(transn, n, kl, ku, &c__1, &AFB(1,1), ldafb, &
			IPIV(1), &WORK(1), n, info);
	    }
	    goto L100;
	}
