...
/*        Loop until stopping criterion is satisfied.   
	...
	csymv_(uplo, n, &q__1, &A(1,1), lda, &X(1,j), &c__1, &
		c_b1, &WORK(1), &c__1);
...
/* L40: */
		...
		i__3 = k + k * a_dim1;
		RWORK(k) = RWORK(k) + ((r__1 = A(k,k).r, dabs(r__1)) + (r__2 
			= r_imag(&A(k,k)), dabs(r__2))) * xk + s;
/* L50: */
	    }
	} else {
	    i__2 = *n;
		...
		xk = (r__1 = X(k,j).r, dabs(r__1)) + (r__2 = r_imag(&X(k,j)), dabs(r__2));
		i__3 = k + k * a_dim1;
		RWORK(k) += ((r__1 = A(k,k).r, dabs(r__1)) + (r__2 = r_imag(&
			A(k,k)), dabs(r__2))) * xk;
		i__3 = *n;
		for (i = k + 1; i <= *n; ++i) {
		    i__4 = i + k * a_dim1;
...
/*           Update solution and try again. */
	    ...
	    csytrs_(uplo, n, &c__1, &AF(1,1), ldaf, &IPIV(1), &WORK(1), 
		    n, info);
...
/*              Multiply by diag(W)*inv(A'). */
		...
		csytrs_(uplo, n, &c__1, &AF(1,1), ldaf, &IPIV(1), &WORK(
			1), n, info);
		i__2 = *n;
		for (i = 1; i <= *n; ++i) {
		    i__3 = i;
