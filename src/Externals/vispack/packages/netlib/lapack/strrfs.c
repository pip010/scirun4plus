...
/*        Compute residual R = B - op(A) * X,   
	...
	strmv_(uplo, trans, diag, n, &A(1,1), lda, &WORK(*n + 1), &c__1);
...
/*              Multiply by diag(W)*inv(op(A)'). */
		...
		strsv_(uplo, transt, diag, n, &A(1,1), lda, &WORK(*n + 1)
			, &c__1);
		i__2 = *n;
		for (i = 1; i <= *n; ++i) {
...
/* L230: */
		...
		strsv_(uplo, trans, diag, n, &A(1,1), lda, &WORK(*n + 1),
			 &c__1);
	    }
	    goto L210;
	}
