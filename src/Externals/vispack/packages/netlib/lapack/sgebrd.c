...
/* Subroutine */ int sgebrd_(integer *m, integer *n, real *a, integer *lda, 
	real *d, real *e, real *tauq, real *taup, real *work, integer *lwork, 
	integer *info)
{
/*  -- LAPACK routine (version 2.0) --   
   ...
   Parameter adjustments   
    ...
    /* Local variables */
    static integer i, j, nbmin, iinfo;
...
/*     Set the block size NB and the crossover point NX.   
   ...
   Computing MAX */
    i__1 = 1, i__2 = ilaenv_(&c__1, "SGEBRD", " ", m, n, &c_n1, &c_n1, 6L, 1L)
	    ;
    nb = max(i__1,i__2);
...
ider using   
		...
		nbmin = ilaenv_(&c__2, "SGEBRD", " ", m, n, &c_n1, &c_n1, 6L, 
			1L);
		if (*lwork >= (*m + *n) * nbmin) {
		    nb = *lwork / (*m + *n);
		} else {
...
d   
	...
	i__3 = *m - i + 1;
	i__4 = *n - i + 1;
	slabrd_(&i__3, &i__4, &nb, &A(i,i), lda, &D(i), &E(i), &
...
ate   
	...
	i__3 = *m - i - nb + 1;
	i__4 = *n - i - nb + 1;
	sgemm_("No transpose", "Transpose", &i__3, &i__4, &nb, &c_b21, &A(i+nb,i), lda, &WORK(ldwrkx * nb + nb + 1), &ldwrky, &
...
/*     Use unblocked code to reduce the remainder of the matrix */
    ...
    i__2 = *m - i + 1;
    i__1 = *n - i + 1;
    sgebd2_(&i__2, &i__1, &A(i,i), lda, &D(i), &E(i), &TAUQ(i), &
	    TAUP(i), &WORK(1), &iinfo);
    WORK(1) = ws;
    return 0;
