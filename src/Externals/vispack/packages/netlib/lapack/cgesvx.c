...
/* Subroutine */ int cgesvx_(char *fact, char *trans, integer *n, integer *
	...
	ipiv, char *equed, real *r, real *c, complex *b, integer *ldb, 
 ...
 */
	...
	cgeequ_(n, n, &A(1,1), lda, &R(1), &C(1), &rowcnd, &colcnd, &
		amax, &infequ);
	if (infequ == 0) {

/*           Equilibrate the matrix. */

	    claqge_(n, n, &A(1,1), lda, &R(1), &C(1), &rowcnd, &colcnd, &
		    amax, equed);
	    rowequ = lsame_(equed, "R") || lsame_(equed, "B");
	    colequ = lsame_(equed, "C") || lsame_(equed, "B");
	}
...
/*        Compute the LU factorization of A. */
	...
	clacpy_("Full", n, n, &A(1,1), lda, &AF(1,1), ldaf);
	cgetrf_(n, n, &AF(1,1), ldaf, &IPIV(1), info);
...
the   
		...
		rpvgrw = clantr_("M", "U", "N", info, info, &AF(1,1), 
			ldaf, &RWORK(1));
		if (rpvgrw == 0.f) {
		    rpvgrw = 1.f;
		} else {
		    rpvgrw = clange_("M", n, info, &A(1,1), lda, &RWORK(
			    1)) / rpvgrw;
		}
		RWORK(1) = rpvgrw;
		*rcond = 0.f;
...
/*     Compute the norm of the matrix A and the   
	...
	*(unsigned char *)norm = 'I';
    ...
    anorm = clange_(norm, n, n, &A(1,1), lda, &RWORK(1));
    rpvgrw = clantr_("M", "U", "N", n, n, &AF(1,1), ldaf, &RWORK(1));
    if (rpvgrw == 0.f) {
	rpvgrw = 1.f;
    } else {
	rpvgrw = clange_("M", n, n, &A(1,1), lda, &RWORK(1)) / 
		rpvgrw;
    }

/*     Compute the reciprocal of the condition number of A. */

    cgecon_(norm, n, &AF(1,1), ldaf, &anorm, rcond, &WORK(1), &RWORK(1),
	     info);
...
/*     Compute the solution matrix X. */
    ...
    cgetrs_(trans, n, nrhs, &AF(1,1), ldaf, &IPIV(1), &X(1,1), ldx,
	     info);

/*     Use iterative refinement to improve the computed solution and   
       compute error bounds and backward error estimates for it. */

    cgerfs_(trans, n, nrhs, &A(1,1), lda, &AF(1,1), ldaf, &IPIV(1),
