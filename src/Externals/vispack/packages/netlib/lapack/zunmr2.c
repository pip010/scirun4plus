...
/* Subroutine */ int zunmr2_(char *side, char *trans, integer *m, integer *n, 
	...
	doublecomplex *c, integer *ldc, doublecomplex *work, integer *info)
{
...
/*        Apply H(i) or H(i)' */
	...
	i__3 = nq - *k + i - 1;
	zlacgv_(&i__3, &A(i,1), lda);
	i__3 = i + (nq - *k + i) * a_dim1;
	aii.r = A(i,nq-*k+i).r, aii.i = A(i,nq-*k+i).i;
	i__3 = i + (nq - *k + i) * a_dim1;
	A(i,nq-*k+i).r = 1., A(i,nq-*k+i).i = 0.;
	zlarf_(side, &mi, &ni, &A(i,1), lda, &taui, &C(1,1), ldc, 
		&WORK(1));
	i__3 = i + (nq - *k + i) * a_dim1;
	A(i,nq-*k+i).r = aii.r, A(i,nq-*k+i).i = aii.i;
	i__3 = nq - *k + i - 1;
	zlacgv_(&i__3, &A(i,1), lda);
/* L10: */
    }
