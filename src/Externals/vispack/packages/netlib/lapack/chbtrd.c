...
/* Subroutine */ int chbtrd_(char *vect, char *uplo, integer *n, integer *kd, 
	complex *ab, integer *ldab, real *d, real *e, complex *q, integer *
...
/*           Reduce to complex Hermitian tridiagonal form, working
 with   
	    ...
	    i__2 = kd1 + ab_dim1;
	    d__1 = AB(kd1,1).r;
	    AB(kd1,1).r = d__1, AB(kd1,1).i = 0.f;
	    i__1 = *n - 2;
	    for (i = 1; i <= *n-2; ++i) {
...
ight */
			    ...
			    i__2 = k - 3;
			    crot_(&i__2, &AB(*kd-k+4,i+k-2), &c__1, &AB(*kd-k+3,i+k-1), &c__1, &D(i + k - 1), &
				    WORK(i + k - 1));
			}
			++nr;
			j1 = j1 - kdn - 1;
...
o diagonal   
		    ...
		    if (nr > 0) {
			clar2v_(&nr, &AB(kd1,j1-1), &AB(kd1,j1), &AB(*kd,j1), &inca,
				 &D(j1), &WORK(j1), &kd1);
		    }
...
tions in Q */
			...
			for (j = j1; i__3 < 0 ? j >= i__2 : j <= i__2; j += 
				i__3) {
			    r_cnjg(&q__1, &WORK(j));
			    crot_(n, &Q(1,j-1), &c__1, &Q(1,j), &c__1, &D(j), &q__1);
/* L30: */
			}
		    }
...
d) outside the band   
			...
			i__5 = j;
			i__6 = (j + *kd) * ab_dim1 + 1;
			q__1.r = WORK(j).r * AB(1,j+*kd).r - WORK(j).i * 
				AB(1,j+*kd).i, q__1.i = WORK(j).r * AB(1,j+*kd)
				.i + WORK(j).i * AB(1,j+*kd).r;
			WORK(j+*kd).r = q__1.r, WORK(j+*kd).i = q__1.i;
			i__4 = (j + *kd) * ab_dim1 + 1;
			i__5 = j;
			i__6 = (j + *kd) * ab_dim1 + 1;
			q__1.r = D(j) * AB(1,j+*kd).r, q__1.i = D(j) * AB(1,j+*kd).i;
			AB(1,j+*kd).r = q__1.r, AB(1,j+*kd).i = q__1.i;
/* L40: */
		    }
/* L50: */
		}
...
/*           make off-diagonal elements real and copy them to E */
		...
		abst = c_abs(&t);
		i__2 = *kd + (i + 1) * ab_dim1;
		AB(*kd,i+1).r = abst, AB(*kd,i+1).i = 0.f;
		E(i) = abst;
		if (abst != 0.f) {
		...
		if (i < *n - 1) {
		    i__2 = *kd + (i + 2) * ab_dim1;
		    i__3 = *kd + (i + 2) * ab_dim1;
		    q__1.r = AB(*kd,i+2).r * t.r - AB(*kd,i+2).i * t.i, q__1.i = AB(*kd,i+2).r * t.i + AB(*kd,i+2).i * t.r;
		    AB(*kd,i+2).r = q__1.r, AB(*kd,i+2).i = q__1.i;
		}
		if (wantq) {
		    r_cnjg(&q__1, &t);
		    cscal_(n, &q__1, &Q(1,i+1), &c__1);
		}
/* L70: */
	    }
	} else {
...
ide */
			...
			i__2 = *kd - 1;
			for (l = 1; l <= *kd-1; ++l) {
			    clartv_(&nr, &AB(kd1-l,j1-kd1+l), &inca, &AB(kd1-l+1,j1-kd1+l), &inca, &D(j1), &WORK(
				    j1), &kd1);
/* L100: */
			}
		    }
...
o annihilate a(i+k-1,i)   
			    ...
			    clartg_(&AB(k-1,i), &AB(k,i), &D(i + k - 1), &WORK(i + k - 1),
				     &temp);
			    i__2 = k - 1 + i * ab_dim1;
...
o diagonal   
		    ...
		    if (nr > 0) {
			clar2v_(&nr, &AB(1,j1-1), &AB(1,j1), &AB(2,j1-1), &
...
tions in Q */
			...
			for (j = j1; i__3 < 0 ? j >= i__2 : j <= i__2; j += 
				i__3) {
			    crot_(n, &Q(1,j-1), &c__1, &Q(1,j), &c__1, &D(j), &WORK(j));
/* L120: */
			}
		    }
...
1) outside the   
			...
			i__5 = j;
			i__6 = kd1 + j * ab_dim1;
			q__1.r = WORK(j).r * AB(kd1,j).r - WORK(j).i * 
				AB(kd1,j).i, q__1.i = WORK(j).r * AB(kd1,j)
				.i + WORK(j).i * AB(kd1,j).r;
			WORK(j+*kd).r = q__1.r, WORK(j+*kd).i = q__1.i;
			i__4 = kd1 + j * ab_dim1;
			i__5 = j;
			i__6 = kd1 + j * ab_dim1;
			q__1.r = D(j) * AB(kd1,j).r, q__1.i = D(j) * AB(kd1,j).i;
			AB(kd1,j).r = q__1.r, AB(kd1,j).i = q__1.i;
/* L130: */
		    }
/* L140: */
		}
...
/*           make off-diagonal elements real and copy them to E */
		...
		if (i < *n - 1) {
		    i__2 = (i + 1) * ab_dim1 + 2;
		    i__3 = (i + 1) * ab_dim1 + 2;
		    q__1.r = AB(2,i+1).r * t.r - AB(2,i+1).i * t.i, q__1.i = AB(2,i+1).r * t.i + AB(2,i+1).i * t.r;
		    AB(2,i+1).r = q__1.r, AB(2,i+1).i = q__1.i;
		}
		if (wantq) {
		    cscal_(n, &t, &Q(1,i+1), &c__1);
		}
/* L160: */
	    }
	} else {
