...
/*  -- LAPACK routine (version 2.0) --   
   ...
   Parameter adjustments   
    ...
    extern /* Subroutine */ int xerbla_(char *, integer *), cunmlq_(
...
/*           Q was determined by a call to CGEBRD with nq >= k */
	    ...
	    cunmqr_(side, trans, m, n, k, &A(1,1), lda, &TAU(1), &C(1,1), ldc, &WORK(1), lwork, &iinfo);
	} else if (nq > 1) {
...
/*           Q was determined by a call to CGEBRD with nq < k */
	    ...
	    i__1 = nq - 1;
	    cunmqr_(side, trans, &mi, &ni, &i__1, &A(2,1), lda, &TAU(1)
...
/*           P was determined by a call to CGEBRD with nq > k */
	    ...
	    cunmlq_(side, transt, m, n, k, &A(1,1), lda, &TAU(1), &C(1,1), ldc, &WORK(1), lwork, &iinfo);
	} else if (nq > 1) {
...
/*           P was determined by a call to CGEBRD with nq <= k */
	    ...
	    i__1 = nq - 1;
	    cunmlq_(side, transt, &mi, &ni, &i__1, &A(1,2), lda,
