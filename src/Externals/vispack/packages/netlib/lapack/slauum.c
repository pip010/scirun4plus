...
/*     Determine the block size for this environment. */
    ...
    nb = ilaenv_(&c__1, "SLAUUM", uplo, n, &c_n1, &c_n1, &c_n1, 6L, 1L);

    if (nb <= 1 || nb >= *n) {

/*        Use unblocked code */

	slauu2_(uplo, n, &A(1,1), lda, info);
    } else {
...
/* Computing MIN */
		...
		strmm_("Right", "Upper", "Transpose", "Non-unit", &i__3, &ib, 
			...
			lda);
		slauu2_("Upper", &ib, &A(i,i), lda, info);
		if (i + ib <= *n) {
		    i__3 = i - 1;
		    i__4 = *n - i - ib + 1;
		    sgemm_("No transpose", "Transpose", &i__3, &ib, &i__4, &
			    ...
			    lda);
		    i__3 = *n - i - ib + 1;
		    ssyrk_("Upper", "No transpose", &ib, &i__3, &c_b15, &A(i,i+ib), lda, &c_b15, &A(i,i), lda);
		}
/* L10: */
	    }
	} else {
...
/* Computing MIN */
		...
		slauu2_("Lower", &ib, &A(i,i), lda, info);
		if (i + ib <= *n) {
		    i__3 = i - 1;
		    i__4 = *n - i - ib + 1;
		    ...
		    i__3 = *n - i - ib + 1;
		    ssyrk_("Lower", "Transpose", &ib, &i__3, &c_b15, &A(i+ib,i), lda, &c_b15, &A(i,i),
			     lda);
		}
/* L20: */
	    }
