...
.   
	    ...
	    i__1 = *n - *m + 1;
	    slarfg_(&i__1, &A(k,k), &A(k,m1), lda, &TAU(
		    k));
...
f A. */
		...
		i__1 = k - 1;
		scopy_(&i__1, &A(1,k), &c__1, &TAU(1), &c__1);

/*              Form   w = a( k ) + B*z( k )  in TAU. */

		i__1 = k - 1;
		i__2 = *n - *m;
		sgemv_("No transpose", &i__1, &i__2, &c_b8, &A(1,m1), lda, &A(k,m1), lda, &c_b8, &TAU(1), &
			c__1);
...
/*              Now form  a( k ) := a( k ) - tau*w   
		...
		i__1 = k - 1;
		r__1 = -(doublereal)TAU(k);
		saxpy_(&i__1, &r__1, &TAU(1), &c__1, &A(1,k), &
			c__1);
		i__1 = k - 1;
		i__2 = *n - *m;
		r__1 = -(doublereal)TAU(k);
		sger_(&i__1, &i__2, &r__1, &TAU(1), &c__1, &A(k,m1)
			, lda, &A(1,m1), lda);
	    }
/* L20: */
	}
