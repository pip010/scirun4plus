...
/*     Check for singularity if non-unit. */
	...
	for (*info = 1; *info <= i__1; ++(*info)) {
	    ...
	    if (A(*info,*info).r == 0.f && A(*info,*info).i == 0.f) {
		return 0;
	    }
/* L10: */
	}
...
/*        Use unblocked code */
	...
	ctrti2_(uplo, diag, n, &A(1,1), lda, info);
    } else {
...
/*              Compute rows 1:j-1 of current block column */
		...
		ctrmm_("Left", "Upper", "No transpose", diag, &i__4, &jb, &
			c_b1, &A(1,1), lda, &A(1,j), lda);
		i__4 = j - 1;
		q__1.r = -1.f, q__1.i = 0.f;
		ctrsm_("Right", "Upper", "No transpose", diag, &i__4, &jb, &
			q__1, &A(j,j), lda, &A(1,j), 
			lda);

/*              Compute inverse of current diagonal block */

		ctrti2_("Upper", diag, &jb, &A(j,j), lda, info);
/* L20: */
	    }
	} else {
...
lumn */
		    ...
		    ctrmm_("Left", "Lower", "No transpose", diag, &i__1, &jb, 
			    &c_b1, &A(j+jb,j+jb), lda, &A(j+jb,j), lda);
		    i__1 = *n - j - jb + 1;
		    q__1.r = -1.f, q__1.i = 0.f;
		    ctrsm_("Right", "Lower", "No transpose", diag, &i__1, &jb,
			     &q__1, &A(j,j), lda, &A(j+jb,j), lda);
		}

/*              Compute inverse of current diagonal block */

		ctrti2_("Lower", diag, &jb, &A(j,j), lda, info);
/* L30: */
	    }
	}
    }
