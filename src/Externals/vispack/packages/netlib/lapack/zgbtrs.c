...
/* Computing MIN */
		...
		l = IPIV(j);
		if (l != j) {
		    zswap_(nrhs, &B(l,1), ldb, &B(j,1), ldb);
		}
		z__1.r = -1., z__1.i = 0.;
		zgeru_(&lm, nrhs, &z__1, &AB(kd+1,j), &c__1, &B(j,1), ldb, &B(j+1,1), ldb);
/* L10: */
	    }
	}
...
/*           Solve U*X = B, overwriting B with X. */
	    ...
	    i__2 = *kl + *ku;
	    ztbsv_("Upper", "No transpose", "Non-unit", n, &i__2, &AB(1,1), ldab, &B(1,i), &c__1);
/* L20: */
	}
...
/*           Solve U**T * X = B, overwriting B with X. */
	    ...
	    i__2 = *kl + *ku;
	    ztbsv_("Upper", "Transpose", "Non-unit", n, &i__2, &AB(1,1),
		     ldab, &B(1,i), &c__1);
/* L30: */
	}
...
/* Computing MIN */
		...
		lm = min(i__1,i__2);
		z__1.r = -1., z__1.i = 0.;
		zgemv_("Transpose", &lm, nrhs, &z__1, &B(j+1,1), ldb,
			 &AB(kd+1,j), &c__1, &c_b1, &B(j,1), ldb);
		l = IPIV(j);
		if (l != j) {
		    zswap_(nrhs, &B(l,1), ldb, &B(j,1), ldb);
		}
...
/*           Solve U**H * X = B, overwriting B with X. */
	    ...
	    i__2 = *kl + *ku;
	    ztbsv_("Upper", "Conjugate transpose", "Non-unit", n, &i__2, &AB(1,1), ldab, &B(1,i), &c__1);
/* L50: */
	}
...
/* Computing MIN */
		i__1 = *kl, i__2 = *n - j;
		lm = min(i__1,i__2);
		zlacgv_(nrhs, &B(j,1), ldb);
		z__1.r = -1., z__1.i = 0.;
		zgemv_("Conjugate transpose", &lm, nrhs, &z__1, &B(j+1,1), ldb, &AB(kd+1,j), &c__1, &c_b1,
			 &B(j,1), ldb);
		zlacgv_(nrhs, &B(j,1), ldb);
		l = IPIV(j);
		if (l != j) {
