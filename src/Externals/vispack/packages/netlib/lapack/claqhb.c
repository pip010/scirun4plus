...
/* Computing MAX */
		...
		for (i = max(1,j-*kd); i <= j-1; ++i) {
		    ...
		    q__1.r = d__1 * AB(*kd+1+i-j,j).r, q__1.i = d__1 * AB(*kd+1+i-j,j).i;
		    AB(*kd+1+i-j,j).r = q__1.r, AB(*kd+1+i-j,j).i = q__1.i;
/* L10: */
		}
		i__4 = *kd + 1 + j * ab_dim1;
		i__2 = *kd + 1 + j * ab_dim1;
		d__1 = cj * cj * AB(*kd+1,j).r;
		AB(*kd+1,j).r = d__1, AB(*kd+1,j).i = 0.f;
/* L20: */
	    }
	} else {
...
/* Computing MIN */
		...
		for (i = j + 1; i <= min(*n,j+*kd); ++i) {
		    ...
		    q__1.r = d__1 * AB(i+1-j,j).r, q__1.i = d__1 * AB(i+1-j,j).i;
		    AB(i+1-j,j).r = q__1.r, AB(i+1-j,j).i = q__1.i;
/* L30: */
		}
/* L40: */
	    }
