...
/*     Determine the block size for this environment. */
    ...
    nb = ilaenv_(&c__1, "CHEGST", uplo, n, &c_n1, &c_n1, &c_n1, 6L, 1L);

    if (nb <= 1 || nb >= *n) {

/*        Use unblocked code */

	chegs2_(itype, uplo, n, &A(1,1), lda, &B(1,1), ldb, info);
    } else {
 ...
 */
		    ...
		    chegs2_(itype, uplo, &kb, &A(k,k), lda, &B(k,k), ldb, info);
		    if (k + kb <= *n) {
			i__3 = *n - k - kb + 1;
			...
			chemm_("Left", uplo, &kb, &i__3, &q__1, &A(k,k), lda, &B(k,k+kb), ldb, 
				&c_b1, &A(k,k+kb), lda)
				;
			i__3 = *n - k - kb + 1;
			q__1.r = -1.f, q__1.i = 0.f;
			cher2k_(uplo, "Conjugate transpose", &i__3, &kb, &
				q__1, &A(k,k+kb), lda, &B(k,k+kb), ldb, &c_b18, &A(k+kb,k+kb), lda);
			i__3 = *n - k - kb + 1;
			q__1.r = -.5f, q__1.i = 0.f;
			chemm_("Left", uplo, &kb, &i__3, &q__1, &A(k,k), lda, &B(k,k+kb), ldb, 
				&c_b1, &A(k,k+kb), lda)
 ...
 */
			...
			i__3 = *n - k - kb + 1;
			q__1.r = -.5f, q__1.i = 0.f;
			chemm_("Right", uplo, &i__3, &kb, &q__1, &A(k,k), lda, &B(k+kb,k), ldb, &
				c_b1, &A(k+kb,k), lda);
			i__3 = *n - k - kb + 1;
			q__1.r = -1.f, q__1.i = 0.f;
			cher2k_(uplo, "No transpose", &i__3, &kb, &q__1, &A(k+kb,k), lda, &B(k+kb,k), ldb, &c_b18, &A(k+kb,k+kb), lda);
			i__3 = *n - k - kb + 1;
			q__1.r = -.5f, q__1.i = 0.f;
			chemm_("Right", uplo, &i__3, &kb, &q__1, &A(k,k), lda, &B(k+kb,k), ldb, &
				c_b1, &A(k+kb,k), lda);
...
,1:k+kb-1) */
		    ...
		    ctrmm_("Left", uplo, "No transpose", "Non-unit", &i__3, &
			    ...
			    lda);
		    i__3 = k - 1;
		    chemm_("Right", uplo, &i__3, &kb, &c_b2, &A(k,k), lda, &B(1,k), ldb, &c_b1, &A(1,k), lda);
		    i__3 = k - 1;
		    cher2k_(uplo, "No transpose", &i__3, &kb, &c_b1, &A(1,k), lda, &B(1,k), ldb, &c_b18,
			     &A(1,1), lda);
		    i__3 = k - 1;
		    chemm_("Right", uplo, &i__3, &kb, &c_b2, &A(k,k), lda, &B(1,k), ldb, &c_b1, &A(1,k), lda);
		    i__3 = k - 1;
...
,1:k+kb-1) */
		    ...
		    ctrmm_("Right", uplo, "No transpose", "Non-unit", &kb, &
			    ...
			    lda);
		    i__3 = k - 1;
		    chemm_("Left", uplo, &kb, &i__3, &c_b2, &A(k,k)
