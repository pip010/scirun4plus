...
/*           Multiply by inv(L). */
	    ...
	    slatrs_("Lower", "No transpose", "Unit", normin, n, &A(1,1), 
		    lda, &WORK(1), &sl, &WORK((*n << 1) + 1), info);

/*           Multiply by inv(U). */

	    slatrs_("Upper", "No transpose", "Non-unit", normin, n, &A(1,1), lda, &WORK(1), &su, &WORK(*n * 3 + 1), info);
	} else {
...
/*           Multiply by inv(L'). */
	    ...
	    slatrs_("Lower", "Transpose", "Unit", normin, n, &A(1,1), 
		    lda, &WORK(1), &sl, &WORK((*n << 1) + 1), info);
	}
