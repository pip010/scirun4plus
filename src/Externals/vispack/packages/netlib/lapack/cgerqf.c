...
/*     Determine the block size. */
    ...
    nb = ilaenv_(&c__1, "CGERQF", " ", m, n, &c_n1, &c_n1, 6L, 1L);
    nbmin = 2;
    nx = 1;
    iws = *m;
    if (nb > 1 && nb < k) {
...
/*        Determine when to cross over from blocked to unblocked code.
   ...
   Computing MAX */
	i__1 = 0, i__2 = ilaenv_(&c__3, "CGERQF", " ", m, n, &c_n1, &c_n1, 6L,
		 1L);
	nx = max(i__1,i__2);
	if (nx < k) {
...
/*           Compute the RQ factorization of the current block   
	    ...
	    i__3 = *n - k + i + ib - 1;
	    cgerq2_(&ib, &i__3, &A(*m-k+i,1), lda, &TAU(i), &WORK(
		    1), &iinfo);
	    if (*m - k + i > 1) {
...
tor   
		...
		i__3 = *n - k + i + ib - 1;
		clarft_("Backward", "Rowwise", &i__3, &ib, &A(*m-k+i,1), lda, &TAU(i), &WORK(1), &ldwork);
...
/*     Use unblocked code to factor the last or only block */
    ...
    if (mu > 0 && nu > 0) {
	cgerq2_(&mu, &nu, &A(1,1), lda, &TAU(1), &WORK(1), &iinfo);
    }
