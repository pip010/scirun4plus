 ...
 *
 *     RCS:  $Id: itclMethod.c,v 1.1.2.48 2009/01/24 19:53:07 wiede Exp $
 * ========================================================================
      ...
      while (argListPtr != NULL) 
      {
        ...
        while (*cPtrPtr != NULL) 
        {
          ...
          if (haveError) 
          {
            ...
            Tcl_AppendResult(interp, startStr, namePtr == NULL ? "??" :
              Tcl_GetString(namePtr),"'s arglist may not contain \"",
...
/*
printf("=================================================\n");
printf("ItclCreateMemberCode\n");
printf("arglist = %s\n",arglist); 
printf("body = %s\n",body); 
printf("mcode->cfunc.argCmd = %lu\n",mcode->cfunc.argCmd);
printf("=================================================\n");
*/
...
/*
printf("------------------------------------------------------------\n");
printf("Itcl_EvalMemberCode\n");
printf("classname = %s\n",imPtr->iclsPtr->namePtr->bytes);
printf("membername = %s\n",imPtr->namePtr->bytes);
if (mcode->flags & ITCL_IMPLEMENT_NONE) printf("flags |= ITCL_IMPLEMENT_NONE\n");
if (mcode->flags & ITCL_IMPLEMENT_TCL) printf("flags |= ITCL_IMPLEMENT_TCL\n");
if (mcode->flags & ITCL_IMPLEMENT_ARGCMD) printf("flags |= ITCL_IMPLEMENT_ARGCMD\n");
if (mcode->flags & ITCL_IMPLEMENT_OBJCMD) printf("flags |= ITCL_IMPLEMENT_OBJCMD\n");

if (imPtr->flags & ITCL_CONSTRUCTOR) printf("flags |= ITCL_CONSTRUCTOR\n");
if (imPtr->flags & ITCL_DESTRUCTOR) printf("flags |= ITCL_DESTRUCTOR\n");
if (imPtr->flags & ITCL_ARG_SPEC) printf("flags |= ITCL_ARG_SPEC\n");
if (imPtr->flags & ITCL_BODY_SPEC) printf("flags |= ITCL_BODY_SPEC\n");
if (imPtr->flags & ITCL_CONINIT) printf("flags |= ITCL_CONINIT\n");
if (imPtr->flags & ITCL_BUILTIN) printf("flags |= ITCL_BUILTIN\n");
if (imPtr->flags & ITCL_COMPONENT) printf("flags |= ITCL_COMPONENT\n");
if (imPtr->flags & ITCL_TYPE_METHOD) printf("flags |= ITCL_TYPE_METHOD\n");
if (imPtr->flags & ITCL_METHOD) printf("flags |= ITCL_METHOD\n");

if (mcode->argumentPtr)
  printf("argumentPtr = %s\n",mcode->argumentPtr->bytes);
if (mcode->bodyPtr)
printf("bodyPtr = %s\n",mcode->bodyPtr->bytes);
