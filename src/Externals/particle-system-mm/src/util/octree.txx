...
float OctreeNode<T>::distanceToClosestPoint( const T* point )
{
  ...
  else
  {
    ...
    iter = nd.begin();
    while ( (iter != nd.end()) && (distance > (*iter)._distance) )
    {
      distance = 
...
//-------------------------------------------------------------------------
template <class T>
void OctreeNode<T>::pointsWithinDistance( const T* point, double distance,
                                          vector<T> &out_points )
{
  // find any points within the distance in this child node
  if ( !gotChildren() )
    for ( int i = 0; i < _num_points; i++ )
    {
      if ( dist(_point[i]->position(),point->position()) < distance )
        out_points.push_back( *_point[i] );
    }

  // otherwise, we need to find which children are within the distance 
  else
  {
    // first, get distances to each child node
    vector<NodeDistance> nd( 8 );
    ...
    {
      _child[(*iter)._node_index].pointsWithinDistance( point,
                                                        distance,
							out_points );
      ++iter;
    }
...
float OctreeNode<T>::distanceToChildNode( int node, const T* point )
{
  ...
  {
    case 0:
      max_val = max(pos(0)-_child[0].getMaxDimension(0), 0.0f);
      distance += max_val*max_val;
      
      max_val = max(pos(1)-_child[0].getMaxDimension(1), 0.0f);
      distance += max_val*max_val;
      
      max_val = max(pos(2)-_child[0].getMaxDimension(2), 0.0f);
      distance += max_val*max_val;
      break;

    case 1:
      max_val = max(_child[1].getMinDimension(0)-pos(0), 0.0f);
      distance += max_val*max_val;
      
      max_val = max(pos(1)-_child[1].getMaxDimension(1), 0.0f);
      distance += max_val*max_val;
      
      max_val = max(pos(2)-_child[1].getMaxDimension(2), 0.0f);
      distance += max_val*max_val;
      break;

    case 2:
      max_val = max(_child[2].getMinDimension(0)-pos(0), 0.0f);
      distance += max_val*max_val;
      
      max_val = max(_child[2].getMinDimension(1)-pos(1), 0.0f);
      distance += max_val*max_val;
      
      max_val = max(pos(2)-_child[2].getMaxDimension(2), 0.0f);
      distance += max_val*max_val;
      break;

    case 3:
      max_val = max(pos(0)-_child[3].getMaxDimension(0), 0.0f);
      distance += max_val*max_val;
      
      max_val = max(_child[3].getMinDimension(1)-pos(1), 0.0f);
      distance += max_val*max_val;
      
      max_val = max(pos(2)-_child[3].getMaxDimension(2), 0.0f);
      distance += max_val*max_val;
      break;

    case 4:
      max_val = max(pos(0)-_child[4].getMaxDimension(0), 0.0f);
      distance += max_val*max_val;
      
      max_val = max(pos(1)-_child[4].getMaxDimension(1), 0.0f);
      distance += max_val*max_val;
      
      max_val = max(_child[4].getMinDimension(2)-pos(2), 0.0f);
      distance += max_val*max_val;
      break;

    case 5:
      max_val = max(_child[5].getMinDimension(0)-pos(0), 0.0f);
      distance += max_val*max_val;
      
      max_val = max(pos(1)-_child[5].getMaxDimension(1), 0.0f);
      distance += max_val*max_val;
      
      max_val = max(_child[5].getMinDimension(2)-pos(2), 0.0f);
      distance += max_val*max_val;
      break;

    case 6:
      max_val = max(_child[6].getMinDimension(0)-pos(0), 0.0f);
      distance += max_val*max_val;
      
      max_val = max(_child[6].getMinDimension(1)-pos(1), 0.0f);
      distance += max_val*max_val;
      
      max_val = max(_child[6].getMinDimension(2)-pos(2), 0.0f);
      distance += max_val*max_val;
      break;

    case 7:
      max_val = max(pos(0)-_child[7].getMaxDimension(0), 0.0f);
      distance += max_val*max_val;
      
      max_val = max(_child[7].getMinDimension(1)-pos(1), 0.0f);
      distance += max_val*max_val;
      
      max_val = max(_child[7].getMinDimension(2)-pos(2), 0.0f);
      distance += max_val*max_val;
      break;
  }
